<?php
	$this->set_css($this->default_theme_path.'/flexigrid-profile/css/flexigrid.css');
	...
	$this->set_js($this->default_theme_path.'/flexigrid-profile/js/cookies.js');
	$this->set_js($this->default_theme_path.'/flexigrid-profile/js/flexigrid.js');
	$this->set_js($this->default_theme_path.'/flexigrid-profile/js/jquery.form.js');
	$this->set_js($this->default_javascript_path.'/jquery_plugins/jquery.numeric.min.js');
	$this->set_js($this->default_theme_path.'/flexigrid-profile/js/jquery.printElement.min.js');
...
<div class="flexigrid" style='width: 100%;' data-unique-hash="<?php echo $unique_hash; ?>">
	...
	<div class="tDiv">
		...
		<div class="tDiv2">
			...
			<div class="pGroup" style="vertical-align: middle !important;">
            	<select id="profile_select">
					<?php foreach($profile_options as $key => $profile){?>
						<option value="<?php echo site_url('admin_company/employee/'.$key)?>" <?php if($key == $default_profile){?> selected="selected" <?php }?>><?php echo $profile; ?>&nbsp;&nbsp;</option>
					<?php }?>
				</select>
			</div>
			...
			<a id="add_mutation" href='javascript:void(0)' onclick="callback_mutation_employee()" title='<?php echo $this->l('list_add'); ?> <?php echo $subject?>' class='add-anchor add_button'>
			<div class="fbutton">
				<div>
					<span class="add-mutasi">Mutasi NIK</span>
				</div>
			</div>
            </a>
		...
		<div class="tDiv3">
			...
			<a id="mutation_history" href='javascript:void(0)' onclick="callback_mutation_history()" title='Lihat sejarah mutasi' class='add-anchor'>
			<div class="fbutton">
				<div>
					<span class="sejarah-mutasi">Sejarah Mutasi</span>
				</div>
			</div>
            </a>
            <div class="btnseparator"></div>
