				...
				<div class='tools'>
					...
					if(!empty($row->action_urls)){
						foreach($row->action_urls as $action_unique_id => $action_url){ 
							...
							if(!empty($action_url) && !is_null($action_url) && $action->css_class !='resume-icon'){
					?>
							<a href="<?php echo $action_url; ?>" class="<?php echo $action->css_class; ?> crud-action" target="_blank" title="<?php echo $action->label?>"><?php 
								if(!empty($action->image_url))
								{
									...
									<span class="<?php echo $action->css_class;?>"></span>									
								<?php }
							?>
						</a>		
					<?php }
						...
						echo '<a href="javascript:void(0)" title="Resume" id="resume_button" onclick="callback_resume_form('.$row->NIK.')"><span class="resume-icon"></span></a>';
					}
