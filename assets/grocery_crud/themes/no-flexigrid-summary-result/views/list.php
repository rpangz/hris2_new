...
<?php foreach($list as $num_row => $row){ ?>
        ...
        <tr rowId="<?php echo $rowId; ?>" <?php if($num_row % 2 == 1){?>class="erow"<?php }?>>
            ...
            <td width='<?php echo $column_width?>%' class='<?php if(isset($order_by[0]) &&  $column->field_name == $order_by[0]){?>sorted<?php }?>'>
                <div relied='<?php echo $column->field_name?>' class='text-left'><?php echo $row->{$column->field_name} != '' ? $row->{$column->field_name} : '&nbsp;' ; ?></div>
            </td>
            <?php }?>
            <?php if(!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)){?>
            <td align="left" width='20%'>
                <div class='tools'>
                    ...
                    <a href="javascript:void(0);" title="Detail" class="btn btn-xs btn-primary btn-flat" onclick="detail_pa('<?php echo $row->KPIID;?>')"><i class="glyphicon glyphicon-file"></i></a>
