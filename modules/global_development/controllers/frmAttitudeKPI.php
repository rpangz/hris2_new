...
class frmAttitudeKPI extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        $crud->add_action('Add', base_url().'assets/grocery_crud/themes/flexigrid/css/images/edit.png', base_url().'assets/grocery_crud/themes/flexigrid/css/images/edit.png',base_url().'assets/grocery_crud/themes/flexigrid/css/images/edit.png',array($this,'modal_dialog_detail'));
        // table name
      

        $crud->set_table($this->cms_complete_table_name('kpi_attitude_master'));        
        ...
        // primary key
        $crud->set_primary_key('attitude_id');

        $crud->set_subject('Attitude KPI');
        ...
        $crud->columns('attitude_periode','attitude_factor','attitude_definition','attitude_jabatan');
        $crud->edit_fields('attitude_periode','attitude_factor','attitude_definition','attitude_jabatan');
        $crud->add_fields('attitude_periode','attitude_factor','attitude_definition','attitude_jabatan');
            
       
       
        $crud->display_as('attitude_periode','Periode');
        $crud->display_as('attitude_factor','Faktor');
        $crud->display_as('attitude_definition','Definisi');
        $crud->display_as('attitude_jabatan','Jabatan');
       
        

        $crud->required_fields('attitude_periode','attitude_factor','attitude_definition','attitude_jabatan');
        ...
        $crud->callback_column('attitude_jabatan', array($this, '_callback_column_attitude_jabatan'));
    ...
    public function index(){
        $crud = $this->make_crud();
        $output = $crud->render();
        $this->view($this->cms_module_path().'/frmAttitudeKPI_view', $output, $this->cms_complete_navigation_name('frmAttitudeKPI'));
    }
    ...
    */
    ...
    public function _callback_edit_field_admin_form_cv($value, $primary_key){
        ...
        $empty_select        = '<div class="row">
                                ...
                                <select data-live-search="true" style="width: 100%; display: none;" class="selectpicker form-control" multiple="multiple" data-container="body" data-header="Pilih Departement" name="dept_id_as_form_cv[]" id="dept_id_as_form_cv[]">';                    
    ...
    public function _callback_edit_field_admin_form_cuti($value, $primary_key){
        ...
        $empty_select        = '<div class="row">
                                <div class="col-md-8">
                                <div class="input-prepend">
                                <select data-live-search="true" style="width: 100%; display: none;" class="selectpicker form-control" multiple="multiple" data-container="body" data-header="Pilih Departement" name="dept_id_as_form_cuti[]" id="dept_id_as_form_cuti[]">';
    ...
    public function _callback_column_attitude_jabatan($value, $row){

        $query  = mysql_query("SELECT * FROM tbl_kpi_attitude_master WHERE attitude_id='".$row->attitude_id."'");
        $total  = mysql_num_rows($query);
        $data   = mysql_fetch_array($query);

        if (!empty($data['attitude_jabatan']) AND !is_null($data['attitude_jabatan'])){

        $i = 1;
        $string = $data['attitude_jabatan'];
        ...
        $i++;
            ...
            if ($total >0){
                $tooltip = '<a href="#" style="text-decoration: none; color:#000000;" class="tip-top" data-toggle="tooltip" title="'.$jobs.'">'.$len.' Jabatan</a>';
            }
            else{
                $tooltip = '';
            }   
    ...
    public function modal_dialog_detail($primary_key , $row){     
        return site_url($this->uri->segment('1').'/'.$this->uri->segment('2').'/index/edit/'.$primary_key);        
