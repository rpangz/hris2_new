...
class frmListCutiUtang extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            ...
            $crud->set_relation('FormCutiNIK', $this->cms_complete_table_name('profile'), '{Nama} - {NIK}',array('NIK >' => 0));
        ...
        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4)) && isset($list)){
            ...
            //$crud->where('tbl_formcuti.companyID !=', 2);
...
*/
        $crud->set_relation('NIK3', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1,'hrd_modules' => $hrd_modules, 'hrd_company' => $companyID));
...
*/
        ...
        if (!is_null($this->input->get('hc')) AND $this->uri->segment(4) !=3){            
            $crud->add_fields('JenisCuti','FormCutiNIK','HakCutiId2','Keperluan','Alamat','NIKPengganti','NoTelpon','Detail','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CreatedBy','CreatedTime','SendMail','companyID');
            $crud->callback_add_field('HakCutiId2', array($this, '_callback_add_field_HakCutiId2'));
        
        }
        elseif (!is_null($this->input->get('hc')) AND $this->uri->segment(4) ==3){
            $crud->add_fields('JenisCuti','FormCutiNIK','HakCutiId2','Keperluan','Alamat','NIKPengganti','NoTelpon','TglActive1','TglActive2','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CreatedBy','CreatedTime','SendMail','companyID');
            $crud->callback_add_field('HakCutiId2', array($this, '_callback_add_field_HakCutiId2'));
            ...
            $crud->add_fields('JenisCuti','FormCutiNIK','HakCutiId','Keperluan','Alamat','NIKPengganti','NoTelpon','Detail','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CreatedBy','CreatedTime','SendMail','companyID');
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/kehadiran/frmListCutiUtang/get_states/', site_url().'/kehadiran/frmListCutiUtang/get_cities/'),
        ...
        foreach($query->result() as $row):
            ...
            elseif ($num_row <=0 || $today > $row->PeriodeExt && !is_null($row->PeriodeExt) || $Sisa <=0){
                $empty_select = "<b>Tidak Ada</b><input type='hidden' maxlength='50' value='' name='HakCutiId'>";
                 ...
                 $empty_select = $Periode1.''.$Periode2.' [Sisa :'.$Sisa.'] - '.$ne."<input type='hidden' maxlength='50' value='".$row->HakId."' name='HakCutiId'>";
    ...
    public function empty_state_dropdown_select(){
        ...
        if(isset($listingID) && $state == "edit"){
            ...
            foreach($db->result() as $row):
                ...
                $dta = mysql_fetch_array(mysql_query('SELECT count(FormCutiNIK) AS QtyPakai FROM tbl_formcuti INNER JOIN 
                                                    tbl_formcutidetail ON tbl_formcuti.CutiId = tbl_formcutidetail.CutiId 
                                                    WHERE tbl_formcuti.FormCutiNIK="'.$FormCutiNIK.'" AND HakCutiId=0 
                                                    AND StatusForm="A" '));
            ...
            if ($today > $soon || $Sisa <= 0){
                ...
                $HakId = $row->HakId;
                $ne    = 'Utang Cuti';
                $link  = 'http://'.$server.'/hris2/kehadiran/frmListCutiUtang/index/'.$listID.'/add/'.$row->HakId;
            }
    ...
    public function get_states(){
        ...
        foreach($db->result() as $row):
           ...
           if (is_null($row->PeriodeExt) OR $row->PeriodeExt ==""){
            ...
            $array[] = array("value" => '?nik='.$FormCutiNIK.'&hc='.$HakId ,"property" => $Periode1.''.$Periode2.'   [sisa : '.$Sisa.' hari]'.' - '.$ne);
        endforeach;
    ...
    public function _callback_add_field_JenisCuti2()    {    
        ...
        if(!is_null($listingID) || $listingID !="" && $state == "add") {
            ...
            $server = $_SERVER['SERVER_NAME'];
                ...
                if($row->id == $listingID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCutiUtang/index/add/'.$row->id.'" selected="selected">'.$row->JenisCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCutiUtang/index/add/'.$row->id.'">'.$row->JenisCutiName.'</option>';
                }
