...
class frmSejarahCutiDirector extends CMS_Priv_Strict_Controller {
    ...
    public function index(){
        ...
        $result = mysql_query("SELECT * FROM tbl_profile INNER JOIN tbl_company ON iCompanyId=CompanyId WHERE NIK=".$this->cms_user_id());
        $data = Array();
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            $data['session_nik']     =  $row['NIK'];
            $data['company_id']      =  $row['CompanyId'];
            //$data['division_id']     =  $row['DivisiID'];
            $data['company_name']   =  $row['cCompanyName'];
        ...
        $this->view($this->cms_module_path().'/frmSejarahCutiDirector_view', $output,$this->cms_complete_navigation_name('frmSejarahCutiDirector'));
    }
