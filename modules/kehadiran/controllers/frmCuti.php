...
class frmCuti extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        $crud->placeholder('Keperluan', 'Minimal 10 karakter...');
        $crud->placeholder('Alamat', 'Minimal 10 karakter...');
        $crud->placeholder('NoTelpon', 'Minimal 10 digit...');

        $crud->help_block('TglMasuk', '(Jumlah cuti harus sama dengan jumlah tanggal yg dipilih...)');
    ...
    public function index(){
        ...
        if($type == 1){
            redirect(site_url('kehadiran/frmCuti'),'refresh');
        }
        ...
        if(isset($listingID) && $state == "add") {
            ...
            elseif ($today > $row->PeriodeExt && !is_null($row->PeriodeExt)){
                ...
                if($row->HakId == $PRD) {
                    ...
                    $empty_select .= '<option value="'.base_url('kehadiran/frmCuti/index/'.$listingID.'/add/'.$row->HakId).'" selected="selected"'.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }else{
                    $empty_select .= '<option value="'.base_url('kehadiran/frmCuti/index/'.$listingID.'/add/'.$row->HakId).'" '.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }
        ...
        $state = $crud->getState();       
            ...
            foreach($db->result() as $row):                                
                if($row->JenisItemCutiId == $itemID) {
                    $empty_select .= '<option value="'.base_url('kehadiran/frmCuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId).'" selected="selected">'.$row->JenisItemCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="'.base_url('kehadiran/frmCuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId).'">'.$row->JenisItemCutiName.'</option>';
                }
