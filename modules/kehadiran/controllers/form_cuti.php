...
class form_cuti extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            ...
            $crud->set_relation('NIK1', $this->cms_complete_table_name('apv_hrd'), '{hrd_name} [{hrd_email}]',array('hrd_status' =>1,'hrd_company' => $my_profile['company'], 'hrd_modules' => $hrd_modules));
        }
        ...
        $crud->display_as('NIK2','Atasan Langsung');
        $crud->display_as('NIK3','Atasan Lebih Tinggi');
        $crud->display_as('NIK1','Verifikator (HRD)');
    ...
    public function index(){
        ...
        $this->view($this->cms_module_path().'/form_cuti_view', $output,
            $this->cms_complete_navigation_name('form_cuti'));
    }
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('form_cuti'),array('CutiId'=>$id));
...
*/           
    ...
    public function _after_update($post_array, $primary_key){
        ...
        include site_url('includes/mailer/form_cuti/SendMailVoid.php?id='.$primary_key);  
...
/*
    ...
    public function _callback_add_field_NIK2(){
        ...
        $empty_select = '<select name="NIK2" class="chosen-select" data-placeholder="Select Atasan Langsung" style="width: 500px; display: none;">';
    ...
    public function _callback_add_field_NIK3(){
        //CREATE THE EMPTY SELECT STRING
        $empty_select = '<select name="NIK3" class="chosen-select" data-placeholder="Select Atasan Lebih Tinggi" style="width: 500px; display: none;">';
        $empty_select_closed = '</select>';
        //GET THE ID OF THE LISTING USING URI
        //$listingID = $this->uri->segment(5);
    ...
    public function _callback_add_field_NIK1(){
        //CREATE THE EMPTY SELECT STRING
        $empty_select = '<select name="NIK1" class="chosen-select" data-placeholder="Select HRD" style="width: 500px; display: none;">';
        $empty_select_closed = '</select>';
        $crud  = new grocery_CRUD();
        $state = $crud->getState();
...
*/
    ...
    public function _callback_add_field_HakCutiId_Melahirkan(){                    
        ...
        return '<strong>3 Month</strong><input type="hidden" maxlength="50" value="90" name="SisaCuti">';
        ...
        if(isset($listingID) && $state == "add") {
            ...
            elseif ($today > $row->PeriodeExt && !is_null($row->PeriodeExt)){
                ...
                if($row->HakId == $PRD) {
                    ...
                    $empty_select .= '<option value="'.base_url('kehadiran/form_cuti/index/'.$listingID.'/add/'.$row->HakId).'" selected="selected"'.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }else{
                    $empty_select .= '<option value="'.base_url('kehadiran/form_cuti/index/'.$listingID.'/add/'.$row->HakId).'" '.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }
    ...
    public function _callback_column_Print($primary_key, $row){
        ...
        if ($row->JenisCuti !=4){
        return "javascript:window.open('".site_url('includes/export/form_cuti/Export2Html2.php') .'?id='.$row->CutiId.'&nik='.$_SESSION['NIK']. "', 'Export', 'width=950, height=' + screen.height + ', top=0, left=0,scrollbars=yes,resizable=yes,copyhistory=no,menubar=no,location=no,toolbar=no,directories=0,titlebar=0,status=no', false);void(0)";        
        }
        else{
        return '#';
        }
        ...
        $state = $crud->getState();       
            ...
            foreach($db->result() as $row):                                
                if($row->JenisItemCutiId == $itemID) {
                    $empty_select .= '<option value="'.base_url('kehadiran/form_cuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId).'" selected="selected">'.$row->JenisItemCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="'.base_url('kehadiran/form_cuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId).'">'.$row->JenisItemCutiName.'</option>';
                }
