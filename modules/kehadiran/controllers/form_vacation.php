 ...
 */
class Form_Vacation extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        $crud->where('FormCutiNIK', $this->cms_user_id());
        ...
        $crud->columns('Keperluan','JenisCuti','Detail','TglMasuk','ApvLevel','StatusForm');
    ...
    public function index(){
        ...
        if ($this->form_validation->run()) {
            ...
            $this->view($this->cms_module_path().'/form_vacation_view', $output,
            $this->cms_complete_navigation_name('form_vacation'));            
        }
    }
    ...
    public function check_registration(){
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($Keperluan == "") {
                ...
                $message = $this->cms_lang("Periode cuti melebihi sisa hak cuti anda, anda hanya boleh mengajukan hutang cuti sesuai tanggal <strong>Cuti Bersama yang ditetapkan oleh Pemerintah</strong>. Untuk mengajukan hutang cuti <br/><br/><a href='javascript:void(0)' onclick='callback_kesepakatan_utang(".$this->cms_user_id().")' title='Sepakat'>Klik Disini</a>.");
                $error = TRUE;
            }
            ...
            /*
            else if($this->Kehadiran_Model->count_form_still_process($this->cms_user_id()) > 0){
    ...
    public function ajax_submit_send_email(){
        $return = $this->Kehadiran_Mailer_Model->mailer_form_cuti('14928', '3866', $level_id=3, $workflow_id=1, $status=1, $act='P', $subject='Process', 2, '');
        echo $return;
    }
    ...
    public function ajax_submit_form_cuti(){
        ...
        if($total_cuti > 0){
            ...
            elseif ($prioritas_cuti == 1){
                $prioritas_cuti__=1;
                $HakCutiId = $this->Kehadiran_Model->allocation_cuti($this->cms_user_id(), 1);
            }
            elseif ($prioritas_cuti == 2) {
                $prioritas_cuti__=2;
                $HakCutiId = $this->Kehadiran_Model->allocation_cuti($this->cms_user_id(), 2);
            }
            else{
                $prioritas_cuti__=1;
                $HakCutiId = 0;
...
/*
            $data_header = array(
                'FormCutiNIK'=> $this->cms_user_id(),
                'EmployeeName'=> $this->Kehadiran_Model->get_data_row($table_name='tbl_profile', $where_column='NIK', $result_column='Nama', $this->cms_user_id()),
                'HakCutiId'=> $HakCutiId,
...
*/
            ...
            /* Alokasi cuti untuk memotong hak cuti
               $type = 1 (Cuti Tahunan)
               $type = 2 (Cuti Besar)
            */
            ...
            foreach ($query1->result() as $key => $credo) {
                    ...
                    $query = $this->db->query("select *,(select count(c.DetailCutiId) as total from ".$this->table_header." as b inner join ".$this->table_detail." as c on b.CutiId=c.CutiId 
                                    ...
                                    tbl_hakcuti as a where a.NIK='".$this->cms_user_id()."' and a.StatusHak=1 and  DATE(a.Periode1) <='".$today."' and 
                    ...
                    foreach ($query->result() as $key => $row) {
                        ...
                        $this->db->limit($credo->Jumlah)
                        ->update($this->table_detail, 
                            array('HakCutiId'=> $row->HakId), 
                            array('CutiId'=> $detail_primary_key,'HakCutiId'=>0, 'AllocationId'=>$credo->AllocationId));
            ...
            $this->Kehadiran_Mailer_Model->mailer_form_cuti($detail_primary_key, $NIK1, $level_id=1, $workflow_id=1, $status=1, $act='P', $subject='Process', $this->company, $comments);
    ...
    public function check_double_date(){
        ...
        $query = $this->db->query("INSERT INTO support_testing VALUES (' test masuk atau tidak =========== ')");  
        $libur_exclude = $this->Kehadiran_Model->check_libur_exclude($this->cms_user_id());
        $query = $this->db->query("INSERT INTO support_testing VALUES (' test masuk atau tidak =========== 2')"); 
        //$libur_exclude = TRUE;
        if(!$libur_exclude) {
            $status_hari_libur = $this->Kehadiran_Model->daftar_hari_libur($sql_date);
        } 
        ...
        elseif(count($status_hari_libur)>0){       
            $status = TRUE;
            $message = 'Tanggal yang dipilih adalah hari libur ( <strong>'.$status_hari_libur[0].'</strong> )';
            $hutang = '';
        }
        else if($sub_total >= $total && $sepakat_utang==0){
            $status = TRUE;
            $message = 'Periode cuti melebihi sisa hak cuti anda, anda hanya boleh mengajukan hutang cuti sesuai tanggal <strong>Cuti Bersama yang ditetapkan oleh Pemerintah</strong>. Untuk mengajukan hutang cuti <br/><a href="javascript:void(0)" onclick="callback_kesepakatan_utang('.$this->cms_user_id().')" title="Sepakat">Klik Disini</a>.';
            $hutang = '';
        }
    ...
    public function ajax_total_sejarah_cuti(){
        ...
        if ($this->input->is_ajax_request()) {   
            ...
            $data = array(  
                "hutang_cuti" => $this->Kehadiran_Model->count_hutang_cuti($this->cms_user_id(),$year), 
    ...
    public function ajax_sejarah_cuti_hutang(){
        ...
        if ($this->input->is_ajax_request()) { 
            ...
            $data = array(                            
                "result" => $this->Kehadiran_Model->sejarah_tanggal_hutang_cuti($this->cms_user_id(),$year),                
            );
            $this->cms_show_json($data);
        }
    }
