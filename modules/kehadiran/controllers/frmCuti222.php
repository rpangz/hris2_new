...
class frmCuti2 extends CMS_Priv_Strict_Controller {
    ...
    public function indexss(){
        ...
        $dd_data = array(
                ...
                //'dd_url' => array('', site_url().'/kehadiran/frmCuti/get_states/', site_url().'/kehadiran/frmCuti/get_cities/'),

                'dd_dropdowns' => array('JenisItemCuti','LocationCuti'),
                //SETUP URL POST FOR EACH CHILD
                //List in order as per above
                'dd_url' => array('', site_url().'/kehadiran/frmCuti2/get_states/', site_url().'/kehadiran/frmCuti2/get_cities/'),
    ...
    public function _before_insert($post_array){
        ...
        $TglActive2 = date('Y-m-d',strtotime('+89 days',$isa));
        $TglMasuk   = date('Y-m-d',strtotime($this->new_format_date_mysql($date=$post_array['TglMasuk'])));

        $end_tgl_cuti = date('d-M-Y',strtotime('+89 days',$isa));
...
*/
    ...
    public function _callback_field_NoTelpon($value, $row){
        return '<div id="tooltip"><input onkeypress="return isNumberKey(event);" maxlength="12" type="text" name="NoTelpon" title="Nomor telpon minimal 10 digit." placeholder="08211321xxxx" class="form-control" value="'.$value.'"></div>';        
    }
...
function toggleFields() {
        ...
        $state = $crud->getState();       
            ...
            foreach($db->result() as $row):                                
                if($row->JenisItemCutiId == $itemID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmCuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId.'" selected="selected">'.$row->JenisItemCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmCuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId.'">'.$row->JenisItemCutiName.'</option>';
                }
    ...
    public function update_tgl_active_dua($primary_key){
        ...
        $TglActive2 = date('Y-m-d',strtotime('+89 days',$isa));
        //$TglMasuk   = date('dd/mm/Y',strtotime($post_array['TglMasuk']));
    ...
    //return '<input type="text" id="TextBox1" />';
  /*
    return '<div class="input-group date" data-provide="datepicker">
                <input type="text" class="form-control" name="TglActive1" id="datepicker_start" placeholder="dd/mm/yyyy">
                <span class="input-group-addon">
                <a href="#" style="text-decoration: none;" class="glyphicon glyphicon-question-sign tip-right" data-toggle="tooltip" title="Tanggal akhir cuti akan dihitung oleh sistem sebanyak 90 hari"></a>
                </span>
            </div>'; */
    //return '<input type="text" id="from" class="form-control" />';
    /*
    return '<div class="input-group date" data-provide="datepicker">
                <input type="text" id="from" class="form-control" name="TglActive1" placeholder="dd/mm/yyyy"/>
                <span class="input-group-addon">
                <a href="#" style="text-decoration: none;" class="glyphicon glyphicon-question-sign tip-right" data-toggle="tooltip" title="Tanggal akhir cuti akan dihitung oleh sistem sebanyak 90 hari"></a>
                </span>
            </div>';
    */
    ...
    public function _callback_add_field_TglMasuk(){
        ...
        /*
        return '<div class="input-group date" data-provide="datepicker">
                <input type="text" class="form-control" name="TglMasuk" id="datepicker_end" placeholder="dd/mm/yyyy">
               <span class="input-group-addon">
                    <a href="#" style="text-decoration: none;" class="glyphicon glyphicon-question-sign tip-right" data-toggle="tooltip" title="Tanggal masuk kerja setelah 90 hari"></a>
                </span>
               </div>';
