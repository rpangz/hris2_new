...
class frmCuti2 extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        elseif ($listingID ==5){
            $crud->unset_add_fields('HakCutiId');
            $crud->field_type('HakCutiId','hidden', 0);
            //$crud->field_type('SisaCuti','hidden', 90);
        ...
        $crud->set_relation('NIKPengganti', $this->cms_complete_table_name('profile'), '{Nama} [{Email}] - {NIK}',array('bStatus' =>1,'DeptID' => $my_profile['dept'], 'NIK !=' => $this->cms_user_id()));
    ...
    public function _after_insert($post_array, $primary_key){
        ...
        if($num_row > 0 || $total >0){
            ...
            include "http://".$_SERVER['SERVER_NAME']."/hris2/includes/mailer/frmCuti2/SendMail.php?id=".$primary_key."&mynik=".$session_nik."&proses=1";
...
*/           
    ...
    public function _after_update($post_array, $primary_key){
        ...
        include "http://".$_SERVER['SERVER_NAME']."/hris2/includes/mailer/frmCuti2/SendMailVoid.php?id=$ID";
        echo "<script language='javascript'>alert('Data Sudah Disimpan dan Email sudah dikirim...');</script>";
...
*/
    ...
    public function _callback_add_field_HakCutiIdxx(){   
        ...
        if(isset($listingID) && $state == "add") {
            ...
            $server = $_SERVER['SERVER_NAME'];
                ...
                if($row->HakId == $PRD) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmCuti2/index/'.$listingID.'/add/'.$row->HakId.'" selected="selected"'.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmCuti2/index/'.$listingID.'/add/'.$row->HakId.'" '.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }
    ...
    public function _callback_column_Print($primary_key, $row){
        return "javascript:window.open('".site_url('includes/export/frmCuti2/Export2Html2.php') .'?id='.$row->CutiId.'&nik='.$_SESSION['NIK']. "', 'Export', 'width=950, height=' + screen.height + ', top=0, left=0,scrollbars=yes,resizable=yes,copyhistory=no,menubar=no,location=no,toolbar=no,directories=0,titlebar=0,status=no', false);void(0)";        
    }


    public function _callback_add_Keperluan(){
        return '<textarea style="width: 500px; height: 80px;" name="Keperluan" class="texteditor"></textarea><span id="characters"><span>';
    }

    public function _callback_field_NoTelpon($value, $row){
        return '<input onkeypress="return isNumberKey(event);" maxlength="12" type="text" name="NoTelpon" placeholder="08211321xxxx" class="form-control" value="'.$value.'">';        
    }
...
function toggleFields() {
        ...
        $state = $crud->getState();       
            ...
            foreach($db->result() as $row):                                
                if($row->JenisItemCutiId == $itemID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmCuti2/index/'.$listingID.'/add/'.$row->JenisItemCutiId.'" selected="selected">'.$row->JenisItemCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmCuti2/index/'.$listingID.'/add/'.$row->JenisItemCutiId.'">'.$row->JenisItemCutiName.'</option>';
                }
    ...
    public function status_form_cuti_melahirkan(){

        $query  = mysql_query("SELECT * FROM tbl_profile WHERE NIK='".$this->cms_user_id()."'");
        $total  = mysql_num_rows($query);
        $data   = mysql_fetch_array($query);

        if ($data['Sex'] =='P' && $data['StatusDiri']==2){
            return '1';
        }else{
            return '0';
        }
