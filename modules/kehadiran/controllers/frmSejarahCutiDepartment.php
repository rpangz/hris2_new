...
class frmSejarahCutiDepartment extends CMS_Priv_Strict_Controller {
    ...
    public function index(){
        ...
        $result = mysql_query("SELECT * FROM tbl_profile INNER JOIN tbl_dept ON iDeptID=DeptID WHERE NIK=".$this->cms_user_id());
        $data = Array();
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            $data['session_nik']    =  $row['NIK'];
            //$data['company_id']     =  $row['CompanyId'];
            $data['department_name']=  $row['cDeptName'];
        ...
        $this->view($this->cms_module_path().'/frmSejarahCutiDepartment_view', $output,$this->cms_complete_navigation_name('frmSejarahCutiDepartment'));
    }
