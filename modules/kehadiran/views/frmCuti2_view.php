...
while ($data = mysql_fetch_array($sql)){
    ...
    if($this->uri->segment('4')==$data['id']){  
      echo '<option value="'.site_url('kehadiran/frmCuti2/index/'.$data['id']).'" SELECTED>'.$data['JenisCutiName'].'</option>';
    }
    else {
      echo '<option value="'.site_url('kehadiran/frmCuti2/index/'.$data['id']).'">'.$data['JenisCutiName'].'</option>';
    }
