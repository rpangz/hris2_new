...
if($state !='edit' AND $state !='add'){
    ...
    if ($jen_cuti == '---ALL---' || is_null($this->uri->segment('3')) || ($this->uri->segment('4')) ==0){
        $notes = 'Silahkan pilih jenis cuti dibawah. Mulai tanggal 7-Mei-2018 Pengajuan cuti Tahunan dan besar menggunakan menu  <a href="{{ base_url }}kehadiran/form_vacation">Form Cuti New</a>';
    }
    elseif($jenis_cuti_id == 3){
        $notes = 'Silahkan lihat info cuti <i>'.$jeniscuti.'</i>. Mulai tanggal 7-Mei-2018 Pengajuan cuti Tahunan dan besar menggunakan menu  <a href="{{ base_url }}kehadiran/form_vacation">Form Cuti New</a>';
    }
...
$sql = mysql_query("SELECT * FROM tbl_jeniscuti WHERE id ".$jen_cuti." and (id=3 or id=5 or id=0) ORDER BY id ASC");					  
while ($data = mysql_fetch_array($sql)){

    if($this->uri->segment('4')==$data['id']){  
      echo '<option value="'.site_url('kehadiran/frmCuti/index/'.$data['id']).'" SELECTED>'.$data['JenisCutiName'].'</option>';
    }
    else {
      echo '<option value="'.site_url('kehadiran/frmCuti/index/'.$data['id']).'">'.$data['JenisCutiName'].'</option>';
    }
