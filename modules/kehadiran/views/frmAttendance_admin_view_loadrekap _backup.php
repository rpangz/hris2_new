...
if ($row_out = $result_out->fetch()){
    ...
    //pengecekan ijin
    $dataijin = "";
    $dataijin = check_ijin($date,$session_nik);
    $countijin = sizeof($dataijin); //untuk mengecek data di dalam array
    if($countijin>0){
      $total_ijin = $total_ijin + 1;
    }

    //pengecekan Alpa
    $N     = "";
    $N     = date('N', strtotime($date));
    if($check_in_txt == '' && $check_in_txt == '' && ($countdatacuti<1 && $countdataliburbersama<1 && $countdatalibur<1 && $countsakittanpasuratdokter<1 && $countsakitdengansuratdokter<1 && $countijin<1) && ($N<>"6" && $N<>"7") ) {
...
<tr>
    ...
    <td class="tg-0laxdetail">Freq</td>
    <td class="tg-0laxdetail">Mnt</td>
    <td class="tg-0laxdetail">%</td>
...
function check_cuti($date,$session_nik){
  ...
  $mysqlcomm = "SELECT a.cutiid,keperluan,tglcuti,tglmasuk FROM tbl_formcuti a, tbl_formcutidetail b 
                         WHERE a.cutiid=b.cutiid AND formcutinik = ".$session_nik." AND statusForm = 'A' AND tglcuti = '".$date."' AND JenisCuti NOT IN (4) ";
...
function check_ijin($date,$session_nik){

  $tgl    = date('Y-m-d', strtotime($date));
  $mysqlcomm = "SELECT * FROM tbl_formijin WHERE JenisIjin = 9 AND statusform = 'A' AND '".$date."' BETWEEN tglactive1 AND tglactive2";
  ...
  while ($data = mysql_fetch_array($query)){
    ...
    $dataijin =         array('IjinId' => $data['IjinId'],
                              'Alasan' => $data['Alasan'],   
                      );
