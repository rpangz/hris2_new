...
echo"<table>
  <tr>
    ...
    <option value='https://$_SERVER[SERVER_NAME]/hris2/kehadiran/frmAttendance/index/0'>---ALL Year ".$current_year."---</option>";
    ...
    if($this->uri->segment('4')==$data['iPeriodId']){  
      echo "<option value='https://$_SERVER[SERVER_NAME]/hris2/kehadiran/frmAttendance/index/$data[iPeriodId]' SELECTED>$data[cPeriodName]</option>";
    }
    else {
      echo "<option value='https://$_SERVER[SERVER_NAME]/hris2/kehadiran/frmAttendance/index/$data[iPeriodId]'>$data[cPeriodName]</option>";
    }
...
/* perubahan data karena sebelumnya di munculkan setahun jadi beratt
if ($periode_id != 0){
    $Periode1 = date('m/j/Y', strtotime($sql_periode['dPeriodStartDate']));
    $Periode2 = date('m/j/Y', strtotime($sql_periode['dPeriodEndDate']));
}
...
echo '<tr>
      ...
      <table>
        <tr>
            ...
            <td>&nbsp;:</td>
            <td style="text-align:right;width:30%;">'.$total_terlambat.'</td>
        </tr>
        <tr>
            <td>Jumlah Cuti</td>
            <td>&nbsp;:</td>
            <td style="text-align:right;width:30%;">'.$total_cuti.'</td>
        </tr>
        <tr>
            <td>Jumlah Libur Nasional</td>
            <td>&nbsp;:</td>
            <td style="text-align:right;width:30%;">'.$total_libur_nasional.'</td>
        </tr>
        <tr>
            <td>Jumlah Libur Bersama</td>
            <td>&nbsp;:</td>
            <td style="text-align:right;width:30%;">'.$total_libur_bersama.'</td>
        </tr>
      </table>
      ';
}
...
$date = date("Y-m-d", strtotime("2017-01-01" . " -1 day"));
$final_date = date("Y-m-d", strtotime("2017-05-01"));
while($date <= $final_date){
   $date = date("Y-m-d", strtotime($date . " +1 day"));
   echo $dates[] = $date.'<br/>';
...
function day_name($date,$session_nik){
...
function format_tanggal($date,$session_nik){
...
function color_td($value,$date,$session_nik){
  ...
  if($countdatacuti>0 || $countdatalibur>0 || $countdataliburbersama>0) {
     return 'yellow';
  }
  else {
    if ($value %2==0){
