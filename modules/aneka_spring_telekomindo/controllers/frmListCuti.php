...
class frmListCuti extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        $crud->add_action('Mail', 'http://'.$_SERVER['SERVER_NAME'].'/hris2/includes/images/print1.png', ' ',' http://'.$_SERVER['SERVER_NAME'].'/hris2/includes/images/print1.png',array($this,'_send_email'));
    ...
    public function _callback_column_Mail($primary_key, $row){
    ...
    public function _callback_column_Run($primary_key, $row){
        ...
        if ($data['StatusForm']=='P'){
            ...
            return "javascript:window.open('".$data['CutiId'].'?id='.$primary_key.'&nik='.$this->cms_user_id(). "');void(0)";
        }
        else{
    ...
    public function _send_email($primary_key , $row){

        return site_url($this->uri->segment('1').'/'.$this->uri->segment('2').'/ajax_submit_send_email/'.$primary_key);
        //return $this->Kehadiran_Mailer_Model->mailer_form_cuti('14487', '1100901', $level_id=1, $workflow_id=1, $status=1, $act='P', $subject='Process', 2, '');        
    }

    public function ajax_submit_send_email($primary_key){
