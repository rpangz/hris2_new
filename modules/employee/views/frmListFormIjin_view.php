...
echo"<table>
  <tr>
    ...
    if($this->uri->segment('4')==$data['JenisIjinId']){  
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$this->uri->segment('1')."/".$this->uri->segment('2')."/index/".$data['JenisIjinId']."/".$NIKKI."' SELECTED>$data[JenisIjinName]</option>";
    }

      else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$this->uri->segment('1')."/".$this->uri->segment('2')."/index/".$data['JenisIjinId']."/".$NIKKI."'>$data[JenisIjinName]</option>";
    }
...
<td>&nbsp</td>
    ...
    echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$this->uri->segment('1')."/".$this->uri->segment('2')."/index/".$JenisIjin."/0' >---ALL---</option>";
    $sql = mysql_query("SELECT * FROM tbl_profile WHERE bStatus =1 ORDER BY Nama ASC");           
     while ($data = mysql_fetch_array($sql)){

     if($this->uri->segment('5')==$data['NIK']){  
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$this->uri->segment('1')."/".$this->uri->segment('2')."/index/".$JenisIjin."/".$data['NIK']."' SELECTED>$data[Nama]</option>";
    }
      else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$this->uri->segment('1')."/".$this->uri->segment('2')."/index/".$JenisIjin."/".$data['NIK']."'>$data[Nama]</option>";
    }
      }
