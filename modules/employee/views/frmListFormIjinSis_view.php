...
if($this->uri->segment('5') !='edit' && $this->uri->segment('5') !='add' && $this->uri->segment('6') !='add'){
  ?>
...
echo"<table style='font-size:12px'>
  ...
  //echo "<option value='http://astapp02/i-MAM2/assets/assets/index/0' >---All---</option>";
    ...
    if($this->uri->segment('4')==$data['JenisIjinId']){  
      //echo "<option value='http://$_SERVER[SERVER_NAME]/hris2/kehadiran/frmListFormIjin/index/$data[JenisIjinId]' SELECTED>$data[JenisIjinName]</option>";
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/kehadiran/frmListFormIjinSis/index/".$data['JenisIjinId']."/".$NIKKI."' SELECTED>$data[JenisIjinName]</option>";
    }

      else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/kehadiran/frmListFormIjinSis/index/".$data['JenisIjinId']."/".$NIKKI."'>$data[JenisIjinName]</option>";
//      echo "<option value='http://$_SERVER[SERVER_NAME]/hris2/kehadiran/frmListFormIjin/index/$data[JenisIjinId]'>$data[JenisIjinName]</option>";
    }
...
echo"</select></td>
    ...
    echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/kehadiran/frmListFormIjinSis/index/".$JenisIjin."/0' >---ALL---</option>";
    $sql = mysql_query("SELECT * FROM tbl_profile WHERE CompanyId =2 ORDER BY Nama ASC");           
     while ($data = mysql_fetch_array($sql)){

     if($this->uri->segment('5')==$data['NIK']){  
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/kehadiran/frmListFormIjinSis/index/".$JenisIjin."/".$data['NIK']."' SELECTED>$data[Nama]</option>";
    }
      else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/kehadiran/frmListFormIjinSis/index/".$JenisIjin."/".$data['NIK']."'>$data[Nama]</option>";
    }
      }
