...
class frmCuti extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if($this->_callback_SisaCuti() <=0 && $list !=3){
            $crud->unset_add();
        }
        ...
        if ($state=='read'){
            ...
            $crud->set_relation('NIKPengganti', $this->cms_complete_table_name('profile'), '{Nama} - {NIK}',array('bStatus' =>1,'DeptID' => $my_profile['dept']));
        }
        ...
        $crud->set_relation('NIKPengganti', $this->cms_complete_table_name('profile'), '{Nama} [{Email}] - {NIK}',array('bStatus' =>1,'DeptID' => $my_profile['dept'], 'bStatus' => 1));
    ...
    public function indexss(){
        ...
        $dd_data = array(
                ...
                'dd_dropdowns' => array('JenisItemCuti','LocationCuti'),
                //SETUP URL POST FOR EACH CHILD
                //List in order as per above
                'dd_url' => array('', site_url().'/employee/frmCuti/get_states/', site_url().'/employee/frmCuti/get_cities/'),
    ...
    public function _after_insert_or_update($post_array, $primary_key){
        ...
        foreach($insert_records as $insert_record){
            ...
            if ($fsize <= $MAXIMUM_FILESIZE && preg_match($rEFileTypes, strrchr($safe_filename, '.'))){
                ...
                $file           = 'http://172.17.0.16/hris2/modules/employee/assets/uploads/'.$file_name;
                $remote_file    = '/hris2/modules/employee/assets/uploads/'.$file_name;
    ...
    public function _callback_add_field_HakCutiIdxx(){   
        ...
        if(isset($listingID) && $state == "add") {
            ...
            $server = $_SERVER['SERVER_NAME'];
                ...
                if($row->HakId == $PRD) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/employee/frmCuti/index/'.$listingID.'/add/'.$row->HakId.'" selected="selected"'.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/employee/frmCuti/index/'.$listingID.'/add/'.$row->HakId.'" '.$sati.' '.$text.'>'.$row->HakId.'. '.$Periode1.' s/d '.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }
        ...
        $state = $crud->getState();       
            ...
            foreach($db->result() as $row):                                
                if($row->JenisItemCutiId == $itemID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/employee/frmCuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId.'" selected="selected">'.$row->JenisItemCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/employee/frmCuti/index/'.$listingID.'/add/'.$row->JenisItemCutiId.'">'.$row->JenisItemCutiName.'</option>';
                }
