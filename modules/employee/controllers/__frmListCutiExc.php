 ...
 */
class frmListCutiExc extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            ...
            $crud->set_relation('FormCutiNIK', $this->cms_complete_table_name('profile'), '{Nama} - {NIK}',array('NIK >' => 0,'companyID >' => 2));
...
*/
    ...
    public function _example_output($output = null){
        ...
        $this->view($this->cms_module_path().'/frmListCutiExc_view', $output,
        $this->cms_complete_navigation_name('frmListCutiExc'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/kehadiran/frmListCutiExc/get_states/', site_url().'/kehadiran/frmListCutiExc/get_cities/'),
    ...
    public function empty_state_dropdown_select(){
        ...
        if(isset($listingID) && $state == "edit"){
            ...
            if ($today > $soon || $Sisa <= 0){
                ...
                $HakId = $row->HakId;
                $ne    = 'Bisa Dipakai';
                $link  = 'http://'.$server.'/hris2/kehadiran/frmListCutiExc/index/'.$listID.'/add/'.$row->HakId;
            }
        ...
        */
        ...
        if(!is_null($listingID) || $listingID !="" && $state == "add") {
            ...
            $server = $_SERVER['SERVER_NAME'];
                ...
                if($row->id == $listingID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCutiExc/index/add/'.$row->id.'" selected="selected">'.$row->JenisCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCutiExc/index/add/'.$row->id.'">'.$row->JenisCutiName.'</option>';
                }
