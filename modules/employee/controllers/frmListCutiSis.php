 ...
 */
class frmListCutiSis extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            ...
            $crud->set_relation('FormCutiNIK', $this->cms_complete_table_name('profile'), '{Nama} - {NIK}',array('CompanyId' => 2));
        ...
        if ($state=='read'){
            $crud->edit_fields('FormCutiNIK','JenisCuti','Keperluan','Alamat','Pengganti','NoTelpon','Detail','TglMasuk','NIK1','NIK2','NIK3');
        }
        else{
...
*/      
        ...
        $crud->set_relation('NIK3', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1, 'hrd_company' => 2));
...
*/
    ...
    public function _example_output($output = null){
        ...
        $this->view($this->cms_module_path().'/frmListCutiSis_view', $output,
        $this->cms_complete_navigation_name('frmListCutiSis'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/kehadiran/frmListCutiSis/get_states/', site_url().'/kehadiran/frmListCutiSis/get_cities/'),
...
/*
        ...
        if ($SendMail ==1){
            include "http://$_SERVER[SERVER_NAME]/hris2/includes/mailer/frmListCutiSis/SendMail.php?id=$ID";
...
*/
    ...
    public function _after_update($post_array, $primary_key){
        ...
        include "http://$_SERVER[SERVER_NAME]/hris2/includes/mailer/frmListCutiSis/SendMailVoid.php?id=$ID";
        echo "<script language='javascript'>alert('Data Sudah Disimpan dan Email sudah dikirim...');history.go(-1);</script>";
    ...
    public function empty_state_dropdown_select(){
        ...
        if(isset($listingID) && $state == "edit"){
            ...
            if ($today > $soon || $Sisa <= 0){
                ...
                $HakId = $row->HakId;
                $ne    = 'Bisa Dipakai';
                $link  = 'http://'.$server.'/hris2/kehadiran/frmListCutiSis/index/'.$listID.'/add/'.$row->HakId;
            }
    ...
    public function empty_city_dropdown_select(){
        ...
        //CREATE THE EMPTY SELECT STRING
       //echo"<script language='javascript'>window.location ='?PRD=5';</script>";
       //redirect('kehadiran/frmListCuti/index/'.$HakCutiId.'/add', 'refresh'); 
    }
        ...
        */
        ...
        if(!is_null($listingID) || $listingID !="" && $state == "add") {
            ...
            $server = $_SERVER['SERVER_NAME'];
                ...
                if($row->id == $listingID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCutiSis/index/add/'.$row->id.'" selected="selected">'.$row->JenisCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCutiSis/index/add/'.$row->id.'">'.$row->JenisCutiName.'</option>';
                }
