 ...
 */
class frmCutiMasal extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('formcutimasal'));
        ...
        $crud->set_subject('Form Cuti Masal');

        $crud->unset_add_fields('CreatedBy','CreatedTime');
        $crud->field_type('CreatedBy', 'hidden', $session_nik);
        $crud->field_type('CreatedTime', 'hidden', $today);


        $crud->unset_add_fields('CreatedBy','CreatedTime','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','NIK1','NIK2','NIK3','Apv1','Apv2','Apv3');
        $crud->field_type('CreatedBy', 'hidden', $session_nik);
        $crud->field_type('CreatedTime', 'hidden', $today);
        ...
        $crud->field_type('NIK1', 'hidden', $session_nik);
        $crud->field_type('NIK2', 'hidden', $session_nik);
        $crud->field_type('NIK3', 'hidden', $session_nik);
        ...
        $crud->columns('CutiId','JenisCuti','StatusForm','Keperluan','Alamat','Detail','TglMasuk');
       
        // displayed columns on add operation
        $crud->add_fields('JenisCuti','Keperluan','Detail','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CreatedBy','CreatedTime');
        $crud->edit_fields('JenisCuti','Keperluan','Detail','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CreatedBy','CreatedTime');
       ...
       // caption of each columns      
        ...
        $crud->display_as('TglMasuk','Tgl Masuk Kerja');
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $crud->required_fields('JenisCuti','StatusForm','Keperluan','Alasan','TglMasuk');
...
*/
        //$crud->set_relation('NIK1', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1));
        //$crud->set_relation('NIK2', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1));
        //$crud->set_relation('NIK3', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1));

        //$crud->set_relation('Apv1', $this->cms_complete_table_name('statusform'), 'NamaStatusForm',array('Id' => 1));
        //$crud->set_relation('Apv2', $this->cms_complete_table_name('statusform'), 'NamaStatusForm',array('Id' => 1));
        //$crud->set_relation('Apv3', $this->cms_complete_table_name('statusform'), 'NamaStatusForm',array('Id' => 1));

        //$crud->set_relation('NIK3', $this->cms_complete_table_name('apv_hrd'), '{hrd_name} [{hrd_email}]',array('hrd_status' => 1));
...
*/
    ...
    public function index(){
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/frmCutiMasal_view', $output,
            $this->cms_complete_navigation_name('frmCutiMasal'));
    }
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('frmCutiMasal'),array('CutiId'=>$id));
    ...
    public function _after_insert($post_array, $primary_key){
        ...
        // HINT : Put your code here
         ...
         //include "http://$_SERVER[SERVER_NAME]/hris2/includes/mailer/frmCuti/SendMail.php?id=$ID&mynik=$session_nik&proses=1";
        //echo "<script language='javascript'>alert('Data Sudah Disimpan dan Email sudah dikirim...$count,$TglCuti');history.go(-1);</script>";


        
        /*
        $sql1 = mysql_query("SELECT * FROM tbl_formcutimasaldetail WHERE CutiId='".$primary_key."' ORDER BY TglCuti ASC");

        while ($data1 = mysql_fetch_array($sql1)){

            //$ins = mysql_fetch_array(mysql_query("SELECT * FROM tbl_hakcuti WHERE PeriodeKerja1 <='$data1[TglCuti]' AND PeriodeKerja2 >='$data1[TglCuti]' AND NIK !=0")); 
            $sql2 = mysql_query("SELECT * FROM tbl_hakcuti WHERE PeriodeKerja1 <='$data1[TglCuti]' AND PeriodeKerja2 >='$data1[TglCuti]' AND NIK !=0"); 
                while ($data2 = mysql_fetch_array($sql2)){

                    $ckc = mysql_query("SELECT * FROM tbl_formcutidetail INNER JOIN tbl_formcuti ON 
                        tbl_formcuti.CutiId=tbl_formcutidetail.CutiId WHERE TglCuti='$data1[TglCuti]' AND FormCutiNIK='$data2[NIK]'"); 

                    $tot      = mysql_num_rows($ckc);
    ...
    public function _before_delete($primary_key){
        ...
        $this->db->delete($this->cms_complete_table_name('formcutimasaldetail'),
              array('CutiId'=>$primary_key));

        $sql = mysql_query("SELECT * FROM tbl_formcutimasaldetail WHERE CutiId='".$primary_key."' ORDER BY TglCuti ASC");
        while ($data = mysql_fetch_array($sql)){
            mysql_query("DELETE FROM tbl_formcutidetail WHERE TglCuti = '$data[TglCuti]'");
        }

        mysql_query("DELETE FROM tbl_formcuti WHERE CutiMasalId = '".$primary_key."'");
    ...
    public function _after_insert_or_update($post_array, $primary_key){
        ...
        foreach($update_records as $update_record){
            ...
            $this->db->update($this->cms_complete_table_name('formcutimasaldetail'),
        ...
        foreach($insert_records as $insert_record){
            ...
            $data['CutiId'] = $primary_key;
            $this->db->insert($this->cms_complete_table_name('formcutimasaldetail'), $data);
    ...
    public function _callback_field_citizen($value, $primary_key){
        ...
        $query = $this->db->select('DetailCutiId, CutiId, Keterangan, TglCuti, active_id')
            ->from($this->cms_complete_table_name('formcutimasaldetail'))
        ...
        return $this->load->view($this->cms_module_path().'/field_cutimasal_detail',$data, TRUE);
    }
    ...
    public function _callback_add_field_HakCutiId(){
        ...
        */
        ...
        if(isset($listingID) && $state == "add") {
            ...
            $dta = mysql_fetch_array(mysql_query('SELECT count(FormCutiNIK) AS QtyPakai FROM tbl_formcutimasal INNER JOIN 
                                                    tbl_formcutimasaldetail ON tbl_formcutimasal.CutiId = tbl_formcutimasaldetail.CutiId 
                                                    WHERE tbl_formcutimasal.FormCutiNIK="'.$session_nik.'" AND HakCutiId="'.$row->HakId.'" 
                                                    AND StatusForm="A" AND JenisCuti="'.$row->JenisHakCuti.'"'));
            $Qty      = $row->Qty;
            ...
            endforeach;
            //$tutup ='<input type="submit" value="Go!" />';
            //RETURN SELECTION COMBO
            //echo $KDFaskes;
    ...
    public function _callback_edit_field_Alasan($value, $row){
        ...
        $query     = $this->db->query('SELECT * FROM tbl_formcutimasal WHERE CutiId='.$ListingID); 
