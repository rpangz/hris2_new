...
class frmListCuti extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            ...
            $crud->set_relation('FormCutiNIK', $this->cms_complete_table_name('profile'), '{Nama} - {NIK}',array('NIK >' => 0,'bStatus' => 1));
        ...
        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4)) && isset($list)){
            ...
            //$crud->where('tbl_formcuti.companyID', 1);
...
*/
        ...
        $crud->columns('FormCutiNIK','NIK','JenisCuti','Keperluan','Detail','TglMasuk','ApvLevel','StatusForm','Aturan','CreatedTime');
       ...
       // caption of each columns      
        ...
        $crud->display_as('TglMasuk','Tgl Kembali Bekerja');
...
*/
        ...
        $crud->set_relation('NIK3', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1,'hrd_company' => $companyID,'hrd_modules' => $hrd_modules));
...
*/
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/kehadiran/frmListCuti/get_states/', site_url().'/kehadiran/frmListCuti/get_cities/'),
    ...
    public function empty_state_dropdown_select(){
        ...
        if(isset($listingID) && $state == "edit"){
            ...
            if ($today > $soon || $Sisa <= 0){
                ...
                $HakId = $row->HakId;
                $ne    = 'Bisa Dipakai';
                $link  = 'http://'.$server.'/hris2/kehadiran/frmListCuti/index/'.$listID.'/add/'.$row->HakId;
            }
        ...
        */
        ...
        if(!is_null($listingID) || $listingID !="" && $state == "add") {
            ...
            $server = $_SERVER['SERVER_NAME'];
                ...
                if($row->id == $listingID) {
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCuti/index/add/'.$row->id.'" selected="selected">'.$row->JenisCutiName.'</option>';
                }else{
                    $empty_select .= '<option value="http://'.$server.'/hris2/kehadiran/frmListCuti/index/add/'.$row->id.'">'.$row->JenisCutiName.'</option>';
                }
    ...
    public function _callback_column_CreatedTime($value, $row){
        ...
        if (!is_null($value) || $value !=""){
            $Jamku       = date('H:i:s',strtotime($value));
            $CreatedTime = date('d-M-Y',strtotime($value));
        }else{
            $Jamku       = '';
            $CreatedTime = '';
        }

        return $CreatedTime.' '.$Jamku;     
