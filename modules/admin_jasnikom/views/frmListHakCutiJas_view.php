...
echo"<table>
  ...
  echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/0' >---ALL---</option>";

    $sql = mysql_query("SELECT * FROM tbl_profile WHERE CompanyId ='".$company_id."' ORDER BY Nama ASC");            
     while ($data = mysql_fetch_array($sql)){

    if($this->uri->segment('4')==$data['NIK']){  
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/$data[NIK]' SELECTED>$data[Nama] - $data[NIK]</option>";
    }

    else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/$data[NIK]'>$data[Nama] - $data[NIK]</option>";
    }
