 ...
 */
class frmKaryawan extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            $crud->set_theme('flexigrid');
            $crud->set_relation('CompanyId', $this->cms_complete_table_name('company'), 'cCompanyCode',array('iCompanyId !=' => 2));
            //$crud->unset_edit();
        }

        else{
            $crud->set_relation('CompanyId', $this->cms_complete_table_name('company'), 'cCompanyName',array('iCompanyId !=' => 2));
        }

        // Periksa PT karyawan
        if ($state =='edit'){            
            $cm        = mysql_fetch_array(mysql_query("SELECT * FROM tbl_profile WHERE NIK=".$listingID));
        ...
        $crud->where('tbl_profile.CompanyId !=', 2);
		...
		$crud->columns('Photos','NIK','Nama','Sex','TglMasuk','Status','CompanyId','DivisiID','DeptID','JabatanID','bStatus','kerja','member');
		
		
        // displayed columns on edit operation
       /*
		$crud->edit_fields('Photos','NIK','NoKTP','TglKTP','Nama','Sex','Agama','TglLahir','TptLahir','Kodepos','Telp',
                            'Hp','Email','AlamatKTP','AlamatDomisili','StatusDiri','TglMenikah','TglMasuk','TglKeluar','BandSkrg','Status',
                            'CompanyId','DivisiID','DeptID','UnitID','SeksiID','JabatanID','JmlAnak','UpdatedBy','UpdatedTime','bStatus','member');
        */

        $crud->edit_fields('Photos','NIK','NoKTP','TglKTP','Nama','Sex','Agama','TglLahir','TptLahir','AlamatKTP','AlamatDomisili','Kodepos','Telp','Hp','BloodType','StatusDiri','NamaIbuKandung','NoNPWP','NoKPJ','NoBPJSKes','EmailPribadi','CompanyId','DivisiID','DeptID','UnitID','SeksiID','JabatanID','JobId','Email','UpdatedBy','UpdatedTime','member','workexp','Education','training','technical','certification','project','Files');
        // displayed columns on add operation
		
        /*

		$crud->add_fields('Photos','NIK','NoKTP','TglKTP','Nama','Sex','Agama','TglLahir','TptLahir','Alamat','Kodepos','Telp',
                            'Hp','Email','AlamatKTP','StatusDiri','TglMenikah','TglMasuk','TglKeluar','BandSkrg','Status',
                            'CompanyId','DivisiID','DeptID','UnitID','SeksiID','JabatanID','JmlAnak','CreatedBy','CreatedTime','UpdatedBy','UpdatedTime','bStatus','member');

        */



        /* Untuk migrasi/update data */

        $crud->add_fields('NIK','Nama','Email','TglMasuk','CompanyId','DivisiID','DeptID','UnitID','SeksiID','CreatedBy','CreatedTime','UpdatedBy','UpdatedTime');
		


        $crud->set_field_upload('Photos','assets/uploads/files');
        $crud->field_type('bStatus', 'true_false', array('Tidak Aktif', 'Aktif'));
        ...
        $crud->display_as('Alamat','Alamat Tinggal');
        $crud->display_as('AlamatDomisili','Alamat Domisili');
		...
		$crud->required_fields('NIK','Nama','TglMasuk','CompanyId','DivisiID','DeptID');
        ...
        $crud->callback_column('kerja',array($this, '_callback_column_kerja'));
    ...
    public function _example_output($output = null){
        ...
        $this->view($this->cms_module_path().'/frmKaryawan_view', $output,
        $this->cms_complete_navigation_name('frmKaryawan'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/karyawan/frmKaryawan/get_states/', site_url().'/karyawan/frmKaryawan/get_cities/', site_url().'/karyawan/frmKaryawan/get_units/',site_url().'/karyawan/frmKaryawan/get_seksi/'),
    ...
    public function _after_insert_or_update($post_array, $primary_key){
        ...
        foreach($insert_records as $insert_record){
            ...
            $this->config =  array(
                  'allowed_types'   => "gif|jpg|png|jpeg|pdf|doc|xml|xls|xlsx",
                  'overwrite'       => TRUE,
                  'max_size'        => "10000KB",
                  'max_height'      => "768",
                  'max_width'       => "1024"   
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //
        // SAVE CHANGES OF family
        //  * The family data in in json format.
        //  * It can be accessed via $_POST['md_real_field_family_col']
        //
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        $data = json_decode($this->input->post('md_real_field_family_col'), TRUE);
        $insert_records = $data['insert'];
        $update_records = $data['update'];
        $delete_records = $data['delete'];
...
// education
    ...
    public function _callback_column_TglMasuk($value, $row){
        ...
        if (is_null($row->TglMasuk) || $row->TglMasuk =='0000-00-00'){
            $Date1 = '';
    ...
    // change dPeriodEndDate format to d-M-Y
    public function _callback_column_kerja($value, $row){
        //return $value." - scale: <b>".$row->date."</b>";
        if (is_null($row->TglMasuk) || $row->TglMasuk =='0000-00-00' || empty($row->TglMasuk)){
            $LamaKerja = '';

        }else{

            $date1 = date('Y-m-d', strtotime($row->TglMasuk));
            $date2 = date('Y-m-d');
    ...
    public function _callback_column_Photos($value, $row){
        ...
        if (isset($value) || !is_null($value)){
            $image = $value;  
            //return "<img src='http://astapp02/hris2/assets/uploads/files/" . $image . "' width=40>";
    ...
    private function randomize_string($value){
        ...
        return substr(md5($value.$time),0,7);
    }
    ...
    public function _callback_column_TermsAndConditions($value, $row){
       return '<div><label><input type="checkbox" id="checkbox1" name="TermsAndConditions" value="1" />I accept the terms and conditions </label></div>
            ...
            <div id="autoUpdate" class="autoUpdate">            
                ...
                Saya Bersedia Email akan dikirimkan ke HRD & Saya, setelah melakukan proses Update.
    ...
    public function _callback_column_FormCV($primary_key, $row){
            ...
            //return "javascript:window.open('".site_url('includes/export/frmKaryawanSis/Export2Html2.php') .'?id='.$row->NIK. "', 'Form CV', 'width=950, height=' + screen.height + ', top=0, left=0,scrollbars=yes,resizable=yes,copyhistory=no,menubar=no,location=no,toolbar=no,directories=0,titlebar=0,status=no', false);void(0)";        
