 ...
 * @author Dompak
 */
class frmKaryawanGroupAdmin extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        $search = $this->input->get('key_words');;

        if (!isset($search)){

            $key_words = '';
            $status_words = 0;              
            $crud->where('tbl_profile.bStatus',1);
            if ($this->max_departement_list_user() >0){
                $where = 'tbl_profile.DeptID IS NOT NULL AND (';
                $where .= $this->departement_list_user();
                $where .= ')';            
                   
                $crud->where($where);
            }
            else{
                $crud->where('tbl_profile.NIK', 0);
            }           
            ...
            $crud->set_primary_key('CertNIK', $this->cms_complete_table_name('profile_certification'));
            $crud->set_relation('NIK', $this->cms_complete_table_name('profile_certification'), '*',array('CertApv'=>'A'));

            //$crud->set_primary_key('TechnicalSkillNIK', $this->cms_complete_table_name('profile_technicalskill'));
            //$crud->set_relation('NIK', $this->cms_complete_table_name('profile_technicalskill'), '*',array('TechnicalSkillStatus'=> 1));



            $crud->where('tbl_profile.bStatus',1);
            $crud->like('tbl_profile.Nama', $key_words);

            $crud->or_like('CertProduct', $key_words);
            $crud->or_like('CertName', $key_words);

            //$crud->or_like('TechnicalSkillName', $key_words);
            //$crud->or_like('TechnicalSkillDesc', $key_words);
        }
        ...
        */
        ...
        /*
        $where .= 'tbl_profile.DeptID=47 ';
        $where .= 'OR tbl_profile.DeptID=46 ';
        $where .= 'OR tbl_profile.DeptID=43 ';
        $where .= 'OR tbl_profile.DeptID=40 ';
        */
...
/*
        if ($this->max_departement_list_user() >0){
            for ($x = 1; $x <= $this->max_departement_list_user(); $x++){
                $crud->or_where('tbl_profile.DeptID',$this->departement_list_user($number= $x));
            }
        }
        else{
            $crud->or_where('tbl_profile.NIK',0);
        } 
*/
        //$this->departement_list_user();
        ...
        // set subject
        $crud->set_subject('Karyawan'.$status_words);

        //$crud->columns('Photos','NIK','Nama','NoKK','NoKTP','TglKTP','Sex','Agama','TglLahir','TptLahir','AlamatKTP','AlamatDomisili','Kodepos','Telp','Hp','BloodType','StatusDiri','NamaIbuKandung','NoNPWP','NoKPJ','NoBPJSKes','EmailPribadi','CompanyId','DivisiID','DeptID','UnitID','SeksiID','JabatanID','Email','member','bStatus','TermsAndConditions','UpdatedTime');
        $crud->columns('NIK','Nama','CompanyId','DivisiID','DeptID');
        ...
        $crud->display_as('UpdatedTime','Updated Time');
        ...
        $crud->callback_column('EmployeeID',array($this,'_callback_column_EmployeeID'));
        ...
        $crud->add_action('Detail Curriculum Vitae', 'http://'.$_SERVER['SERVER_NAME'].'/hris2/includes/images/cv4.png', ' ',' http://'.$_SERVER['SERVER_NAME'].'/hris2/includes/images/cv4.png',array($this,'_callback_column_FormCV'));
    ...
    public function _example_output($output = null){
        $data = array(
            ...
            'nikki' => $this->check_departement_list_user(),
            'session_nik' => $this->cms_user_id(),
        ...
        $this->view($this->cms_module_path().'/frmKaryawanGroupAdmin_view', $output,
        $this->cms_complete_navigation_name('frmKaryawanGroupAdmin'));    
    }
...
// education
        ...
        $update_records = $data['update'];
        $delete_records = $data['delete'];
        $real_column_names = array('CertId', 'CertNIK', 'CertDate', 'CertProduct', 'CertName', 'CertPartnerName','CertValidStart','CertValidFinish','CertFileName','CertFileUrl','CertStatus','CertApv','CertProcessId','CertUpdatedBy');
        $set_column_names = array();
    ...
    public function _callback_column_EmployeeID($value, $row){       
    ...
    public function departement_list_user(){

        $sql    = mysql_query('select * from tbl_group_admin WHERE admin_group_nik="'.$this->cms_user_id().'" AND admin_form_cv IS NOT NULL');
        ...
        while ($data = mysql_fetch_array($sql)){
            ...
            if ($no==1){
                $where .= 'tbl_profile.DeptID='.$data['admin_form_cv'].' ';
            }
            else{
                $where .= 'OR tbl_profile.DeptID='.$data['admin_form_cv'].' ';
            }
    ...
    public function departement_list_usersss($number){

        $sql    = mysql_query('select @rownum:=@rownum+1 No,admin_group_nik,admin_form_cv from tbl_group_admin, (SELECT @rownum:=0) r WHERE admin_group_nik="'.$this->cms_user_id().'" AND admin_form_cv IS NOT NULL');
        $total  = mysql_num_rows($sql);        
        //$data   = mysql_fetch_array($sql);
    ...
    public function max_departement_list_user(){
    ...
    public function check_departement_list_user(){
        ...
        $sql    = mysql_query('SELECT * FROM tbl_group_admin INNER JOIN tbl_profile ON admin_form_cv=DeptID WHERE admin_group_nik="'.$this->cms_user_id().'" AND NIK='.$nik.' AND admin_form_cv IS NOT NULL');
        $total  = mysql_num_rows($sql);
