...
*/
        ...
        $crud->columns('NIK','Nama','CompanyId','certification','technicalskill');
        ...
        $crud->display_as('technicalskill','Skill');
        $crud->display_as('certification','Certification');
        ...
        $crud->callback_column('certification',array($this,'_callback_column_certification'));
        $crud->callback_column('technicalskill',array($this,'_callback_column_technicalskill'));
        ...
        $crud->add_action('Detail  ', '', '','',array($this,'_callback_column_FormCV'));
...
// education
    ...
    // returned on view
    public function ___callback_column_certification($value, $row){
        $module_path = $this->cms_module_path();
        $query = $this->db->select('CertId,CertNIK,CertDate,CertItem,CertProduct,CertName,CertPartnerName,CertValidStart,CertValidFinish,CertFileName,CertFileUrl,CertStatus,CertApv,CertProcessId,CertUpdatedBy')
    ...
    public function _callback_column_certification($value, $row){
        $query = $this->db->select('*')
               ->from('tbl_profile')
               ->join('tbl_profile_certification', 'CertNIK=NIK AND CertProcessId=ProcessCVNumber')
               ->where('NIK', $row->NIK)
               ->order_by('CertDate','ASC')
               ->get();
        ...
        foreach($query->result() as $data){
            $text .= $data->CertName.' ('.$data->CertProduct.')';
            if ($no < $num_row){
                $text .= ', ';
            }
    ...
    public function _callback_column_technicalskill($value, $row){
        $query = $this->db->select('*')
               ->from('tbl_profile')
               ->join('tbl_profile_technicalskill', 'TechnicalSkillNIK=NIK AND TechnicalSkillProcessId=ProcessCVNumber')
               ->where('NIK', $row->NIK)
               ->order_by('TechnicalSkillName','ASC')
               ->get();
        ...
        $no      = 1;
        foreach($query->result() as $data){
            $text .= $data->TechnicalSkillName.' ('.$data->TechnicalSkillDesc.')';
            if ($no < $num_row){
                $text .= ', ';
            }
