...
echo"
    ...
    <a href='https://$_SERVER[SERVER_NAME]/hris2/karyawan/frmKaryawanSisindokom/index/edit/".$NIK."/?state=last_update&id=$process_id'><i>see the last update form</i></a>
    ...
    <tbody>
        ...
        <div class="sectionTitle">
            ...
            <table class="layout display responsive-table">
                ...
                <?php 
                    $mysqlcomm = "SELECT *,
                        CASE WHEN certvalidstart IS NULL THEN 'N.A' ELSE DATE_FORMAT(certvalidstart,'%d-%b-%Y') END validstart,
                        CASE WHEN certvalidfinish IS NULL THEN 'N.A' ELSE DATE_FORMAT(certvalidfinish,'%d-%b-%Y')  END validfinish
                        FROM tbl_profile_certification WHERE CertNIK=".$nikki."  ORDER BY CertDate ASC";
                    $Cert = mysql_query($mysqlcomm);
                    //echo $mysqlcomm;
                    ...
                    while ($ws5 = mysql_fetch_array($Cert)){
                        ...
                        $CertValidStart  = $ws5['validstart'];
                        $CertValidFinish = $ws5['validfinish'];
                        //echo "TEST : ".strtotime($ws5['CertValidFinish']);
        ...
        <div class="sectionTitle">
            ...
            <table class="layout display responsive-table">
                ...
                <?php 
                    $project = mysql_query("SELECT *,DATE_FORMAT(ProjectDate,'%M %Y') projectdatebln FROM tbl_profile_projecthistory WHERE ProjectNIK=".$nikki."  ORDER BY ProjectDate ASC");
                    while ($ws6 = mysql_fetch_array($project)){
                        $ProjectDate =date('d-M-Y', strtotime($ws6['ProjectDate']));
                        $ProjectBln = $ws6['projectdatebln'];
                ...
                ?>
                    <tr>
                        <td class="organisationnumber" width="90px"><?php echo $ProjectBln ?></td>
