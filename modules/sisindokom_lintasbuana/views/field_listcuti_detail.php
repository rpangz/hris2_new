<?php
    ...
    //$PRD       = $this->uri->segment(7);

    

    if (!is_null($this->input->get('hc'))){
        $PRD = $this->input->get('hc');
    ...
    $dta = mysql_fetch_array(mysql_query('SELECT count(FormCutiNIK) AS QtyPakai FROM tbl_formcuti INNER JOIN 
                                                    ...
                                                    WHERE tbl_formcuti.FormCutiNIK="'.$NIKKI.'" AND HakCutiId="'.$PRD.'" 
                                                    AND StatusForm="A" AND JenisCuti'.$JenisCuti));
            
    $dta2 = mysql_fetch_array(mysql_query('SELECT * FROM tbl_hakcuti WHERE HakId="'.$PRD.'"'));
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        if($("tr.md_field_citizen_tr").length<'<?php echo $Sisa ?>')
            // new data
        var data = new Object();
          //data.KKWTId    = ''; 
          data.Keterangan = '';
    ...
    $(document).ajaxSuccess(function(event, xhr, settings) {
        if (settings.url == "{{ module_site_url }}frmListCuti/index/insert") {
