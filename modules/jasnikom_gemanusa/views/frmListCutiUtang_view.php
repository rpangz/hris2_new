...
echo"<table>
  ...
  //echo "<option value='http://astapp02/i-MAM2/assets/assets/index/0' >---All---</option>";
    ...
    if($this->uri->segment('4')==$data['id']){  
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/".$data['id']."/".$NIKKI."' SELECTED>$data[JenisCutiName]</option>";

    }

    else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/".$data['id']."/".$NIKKI."'>$data[JenisCutiName]</option>";
    }
...
<td>&nbsp</td>
    ...
    echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/".$JenisCuti."/0' >---ALL---</option>";
    $sql = mysql_query("SELECT * FROM tbl_profile WHERE CompanyId ='$company_id' ORDER BY Nama ASC");           
     while ($data = mysql_fetch_array($sql)){

     if($this->uri->segment('5')==$data['NIK']){  
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/".$JenisCuti."/".$data['NIK']."' SELECTED>$data[Nama]</option>";
    }
      else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/".$modules."/".$this->uri->segment('2')."/index/".$JenisCuti."/".$data['NIK']."'>$data[Nama]</option>";
    }
      }
