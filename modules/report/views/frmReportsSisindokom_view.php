...
echo'<table>
  <tr>
    <td>
    <select class="form-control" id=select onchange=goToPage("select")>
  <option value="" disabled SELECTED>Select Report</option>';

    $sql = mysql_query("SELECT * FROM tbl_reports WHERE ReportStatus=1 ORDER BY ReportId ASC");           
     while ($data = mysql_fetch_array($sql)){

    if($this->uri->segment('4')== $data['ReportId']){  
      echo '<option value="http://'.$_SERVER['SERVER_NAME'].'/hris2/report/frmReportsSisindokom/index/'.$data['ReportId'].'" SELECTED>'.$data['ReportName'].'</option>';
    }
    else {
      echo '<option value="http://'.$_SERVER['SERVER_NAME'].'/hris2/report/frmReportsSisindokom/index/'.$data['ReportId'].'">'.$data['ReportName'].'</option>';
    }
...
echo'</select></td>
    <td>&nbsp;&nbsp;
      <a href="#myModal" class="btn btn-primary" data-toggle="modal" '.modal_status_btn($this->uri->segment('4')).'>Report Filters</a>
    </td>
    </tr>
    </table><br/>';
*/
...
<script src="https://<?php echo $_SERVER['SERVER_NAME']?>/hris2/includes/js/bootstrap-datepicker.js"></script>
    ...
    <div class="modal-body">
      ...
      <form class="form-horizontal" role="form" action="" enctype="multipart/form-data" method="get">
        <table width="100%" border="0" cellspacing="0" cellpadding="0"> 
         <tr>
          <td>
           <?php include 'https://'.$_SERVER['SERVER_NAME'].'/hris2/includes/report/modul_item.php?company='.$company_id;?>
          </td>
        </tr>     
       </table>        
      <br/>
...
if (!is_null($this->input->get('submit')) && ($this->input->get('submit')=='Generate')){
  ...
  include 'https://'.$_SERVER['SERVER_NAME'].'/hris2/includes/report/'.report_modules_name($this->uri->segment('4')).'?company='.$company_id.'&pt='.$this->input->get('pt').'&div='.$this->input->get('div').'&dept='.$this->input->get('dept').'&jc='.$this->input->get('jc').'&from='.$this->input->get('from').'&to='.$this->input->get('to');
