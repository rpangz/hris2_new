...
class report_karyawan_mutasi extends CMS_Priv_Strict_Controller {
    ...
    public function _example_output($output = null)
    {
        ...
        $this->view($this->cms_module_path().'/report_karyawan_mutasi_view', $output,
        $this->cms_complete_navigation_name('report_karyawan_mutasi'));    
    }
    ...
    public function index(){
        ...
        */
        ...
        $output = $crud->render();
        $output->company = $this->get_company();
        $output->jenis_mutasi = $this->get_jenis_mutasi();        
        $this->view($this->cms_module_path().'/report_karyawan_mutasi_view', $output,
        $this->cms_complete_navigation_name('report_karyawan_mutasi'));
     ...
     public function get_jenis_mutasi()
    {
        ...
        $this->db->select("*")
                 ->from('tbl_master_pergerakan_mutasi')
                 ->where('status', '1');
        $db = $this->db->get();
        
        $data = '';
        foreach($db->result() as $row):
           $data .= '<option value="'.$row->mutasi_id.'">'.$row->mutasi_name.'</option>';
        endforeach;
    ...
    public function ajax_load_data(){
        ...
        if($tglfr==''){
            $connscr .= ' AND tglmasuk>=STR_TO_DATE("01/01/1900","%d/%m/%Y")';
        } else {
            $connscr .= ' AND tglmasuk>=STR_TO_DATE("'.$tglfr.'","%d/%m/%Y")';
        }

        if($tglto==''){
            $connscr .= ' AND tglmasuk<=STR_TO_DATE("31/12/5000","%d/%m/%Y")';
        } else {
            $connscr .= ' AND tglmasuk<=STR_TO_DATE("'.$tglto.'","%d/%m/%Y")';
        }
        ...
        $corequery =   'SELECT a.*,
                        b.cCompanyCode AS companycodelama,b.cCompanyName AS companynamelama,
                        c.cCompanyCode AS companycodebaru,c.cCompanyName AS companynamebaru FROM (
                        SELECT DATE_FORMAT(created_time,"%d/%m/%Y") tanggal,nama_karyawan,
                        nik_lama,b.companyID AS companylama,nik_baru,c.companyID AS companybaru,mutasi_name
                        FROM tbl_profile_pergerakan_mutasi a
                        LEFT JOIN tbl_profile b ON a.nik_lama=b.nik
                        LEFT JOIN tbl_profile c ON a.nik_baru=c.nik
                        LEFT JOIN tbl_master_pergerakan_mutasi d ON a.mutasi_id=d.mutasi_id
                        ) a
                        LEFT JOIN tbl_company b ON a.companylama=b.iCompanyID
                        LEFT JOIN tbl_company c ON a.companybaru=c.iCompanyID                        
                        ';
            ...
            if($grouping<>'all'){
                ...
                {       
                        ...
                        {  
                            ...
                            $loaddatadetail .= '<tr>
                                                    ...
                                                    <td>'.$rowdetail->nik_lama.'</td>
                                                    <td>'.$rowdetail->companycodelama.'</td>
                                                    <td>'.$rowdetail->nik_baru.'</td>
                                                    <td>'.$rowdetail->companycodebaru.'</td>
                                                    <td>'.$rowdetail->mutasi_name.'</td>
                                                </tr> 
                                                ';
                        }    

                        $loaddatadetail .= '<tr><td colspan=7 style="font-weight:bold; text-align: center">Total Karyawan: </td><td style="text-align:right; font-weight:bold; padding-right: 5px;">'.$grouptotal.'</td></tr>';
                ...
                $loaddatadetail .= '
                                    <table class="ReportTable"> 
                                        <tbody>
                                            <tr><td style="text-align: center; font-weight: bold;">Grand Total Karyawan: '.$alltotal.'</td></tr>
                                        </tbody>     
                                    </table>';
                    ...
                    $loaddatadetail .= '<table class="ReportTable" id="datareportall">
                        ...
                        $loaddatadetail .= '</tbody></table><br>';
                        //$loaddatadetail .= '<tr><td>Total Karyawan: '.$alltotal.'</td></tr><br><br>';  
                        $loaddatadetail .= '
                                    <table class="ReportTable"> 
                                        <tbody>
                                            <tr><td style="text-align: center; font-weight: bold;">Grand Total Karyawan: '.$alltotal.'</td></tr>
