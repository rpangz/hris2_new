...
class report_karyawan_masuk extends CMS_Priv_Strict_Controller {
    ...
    public function ajax_load_data(){
        ...
        $corequery = 'SELECT a.companyid,deptid,DivisiId,cCompanyName,cCompanyCode,
                      ...
                      CASE WHEN cDeptName IS NULL THEN "" ELSE cDeptName END cDeptName,nik,nama,noKTP,tgllahir,tptlahir,DATE_FORMAT(tglmasuk,"%d/%m/%Y") AS tglmasuk,BandSkrg,Grade,NamaJabatan,StatusName,
                      CASE WHEN a.bstatus = 1 THEN "ACTIVE" ELSE "NON ACTIVE" END bstatus,
                      DATE_FORMAT(tgllahir,"%d/%m/%Y") AS tgllahirformat,
                      CASE WHEN FLOOR(DATEDIFF(current_date(), tgllahir)/365) IS NULL THEN 0 ELSE FLOOR(DATEDIFF(current_date(), tgllahir)/365) END AS usia,
                      CASE WHEN g.remarks IS NULL THEN "" ELSE g.remarks END ketusia
                      FROM tbl_profile a
                      ...
                      LEFT JOIN tbl_spek_golongan_usia g ON FLOOR(DATEDIFF(current_date(), tgllahir)/365) >= g.usia_from
                                      AND FLOOR(DATEDIFF(current_date(), tgllahir)/365) <= g.usia_to
                      WHERE 1=1 '.$connscr.' '.$ordering;
                ...
                {       
                        ...
                        {  
                            ...
                            $loaddatadetail .= '<tr>
                                                    ...
                                                    <td>'.$rowdetail->noKTP.'</td>
                                                    <td>'.$rowdetail->tglmasuk.'</td>
                                                    <td>'.$rowdetail->tgllahirformat.'</td>
                                                    <td>'.$rowdetail->usia.'</td>
                                                    <td>'.$rowdetail->cCompanyCode.'</td>
                                                    <td>'.$rowdetail->cDivName.'</td>
                                                    <td>'.$rowdetail->cDeptName.'</td>
                                                    <td>'.$rowdetail->BandSkrg.'/'.$rowdetai->Grade.'</td>
                                                    <td>'.$rowdetail->NamaJabatan.'</td>
                                                    <td>'.$rowdetail->StatusName.'</td>
                                                    <td>'.$rowdetail->bstatus.'</td>
                                                </tr> 
                                                ';
                        }    

                        $loaddatadetail .= '<tr><td colspan=13 style="font-weight:bold; text-align: center">Total Karyawan: </td><td style="text-align:right; font-weight:bold; padding-right: 5px;">'.$grouptotal.'</td></tr>';
