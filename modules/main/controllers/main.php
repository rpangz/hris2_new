...
class Main extends CMS_Controller
{
    ...
    public function login()
    {
        ...
        $allow_forgot = $this->cms_allow_navigate('main_forgot_fill_identity');
        ...
        if ($this->form_validation->run()) {
            ...
            $user_name = strtolower($identity);
            mysql_query("UPDATE tbl_main_user SET dUserUpdateTime = '".$today."' WHERE LOWER(user_name) = '".$user_name."'");
                ...
                $data = array(
                    ...
                    "forgot_caption" => $this->cms_lang("Forgot Password"),
        ...
        if (isset($activation_code)) {
            ...
            if ($this->form_validation->run()) {
                if ($this->cms_generate_activation_code($identity, TRUE, 'FORGOT')) {
                    ...
                    print '<script language="javascript">alert("Email sudah dikirim...");history.go(-1);</script>';
    ...
    public function register(){
        ...
        $honey_pot_pass =
            (strlen($this->input->post('user_name', ''))==0) &&
            ...
            (strlen($this->input->post('user_id', ''))==0);
        if(!$honey_pot_pass){
            show_404();
            die();
        }
        ...
        $user_id          = $this->input->post($previous_secret_code.'user_id');

        $company          = mysql_fetch_array(mysql_query('SELECT * FROM tbl_profile WHERE NIK="'.$user_id.'"'));

        if ($company['CompanyId'] == 2){
            mysql_query("INSERT INTO tbl_main_group_user (group_id,user_id) VALUES ('11','".$user_id."')");
        }
        elseif ($company['CompanyId'] == 3){
            mysql_query("INSERT INTO tbl_main_group_user (group_id,user_id) VALUES ('11','".$user_id."')");
        }
        elseif ($company['CompanyId'] == 4){
            mysql_query("INSERT INTO tbl_main_group_user (group_id,user_id) VALUES ('11','".$user_id."')");
        }
        elseif ($company['CompanyId'] == 5){
            mysql_query("INSERT INTO tbl_main_group_user (group_id,user_id) VALUES ('11','".$user_id."')");
        }

        //set validation rule
        $this->form_validation->set_rules($previous_secret_code.'user_id', 'NIK', 'required|xss_clean');
        ...
        if ($this->form_validation->run()  && !$this->cms_is_user_exists($user_name) && !$this->cms_is_user_exists($user_id)) {
            $this->cms_do_register($user_name, $email, $real_name, $password);

            
            $user_id_update = array("user_id" => $user_id);
            $this->db->update(cms_table_name('main_user'),$user_id_update,array('user_id' => 0));
    ...
    public function check_registration(){
        if ($this->input->is_ajax_request()){
            ...
            $user_id_exists      = $this->cms_is_user_exists($user_id);
            */
            ...
            $tampil = mysql_query("SELECT * FROM tbl_main_user WHERE user_id =".$NIK);
            $total  = mysql_num_rows($tampil);

            $dw     = mysql_query("SELECT * FROM tbl_profile WHERE NIK =".$NIK);
            $jml    = mysql_num_rows($dw);

            $user_id_exists      = $total;
            $user_id_exists_jml  = $jml;
            ...
            if ($user_id == ""){
                //$message = $this->cms_lang("NIK is empty");
                $message = $this->cms_lang("NIK kosong");
                $error = TRUE;
            } else if ($user_id_exists) {
                //$message = $this->cms_lang("NIK already exists");
                $message = $this->cms_lang("NIK sudah terdaftar");
                $error = TRUE;
            }else if ($user_id_exists_jml == 0) {
                //$message = $this->cms_lang("NIK already exists");
                $message = $this->cms_lang("NIK belum terdaftar sebagai karyawan ASTEL Group, Silahkan hubungi Administrator");
                $error = TRUE;
            } else if ($user_name == "") {
                //$message = $this->cms_lang("Username is empty");
                $message = $this->cms_lang("Username kosong");
                $error = TRUE;
            } else if ($user_name_exists) {
                //$message = $this->cms_lang("Username already exists");
                $message = $this->cms_lang("Username sudah terdaftar");
                $error = TRUE;
            } else if (!$valid_email){
                //$message = $this->cms_lang("Invalid email address");
                $message = $this->cms_lang("Invalid alamat email");
                $error = TRUE;
            } 
            /*
            else if ($email_exists){
                //$message = $this->cms_lang("Email already used");
                $message = $this->cms_lang("Email sudah terdaftar");
                $error = TRUE;
            }
            */

            $data = array(
                "exists" => $user_id_exists || $user_name_exists,
                "error" => $error,
                "message" => $message
            );
    ...
    public function change_profile(){
        ...
        $SQL   = "SELECT user_name, email, real_name, photo FROM ".cms_table_name('main_user')." WHERE user_id = " . $this->cms_user_id();
        ...
        if ($this->form_validation->run()) {
            ...
            {   
                ...
                if(file_exists(FCPATH.'assets/images/avatar/'.$this->input->post('remove_photo')) && $this->input->post('remove_photo'))
                    unlink(FCPATH.'assets/images/avatar/'.$this->input->post('remove_photo'));
                //$data['userfile'] = '';

                $this->db->update('tbl_main_user',array('photo'=> ''), array('user_id'=> $this->cms_user_id()));               
            ...
            if(!empty($_FILES['userfile']['name'])){
                ...
                $config = array('upload_path' => FCPATH.'assets/images/avatar/',
                ...
                $upload_path = FCPATH.'assets/images/avatar/';

                //$thumbnail_name = 'thumb_'.$fileName;
                $this->image_moo->load($upload_path.$fileName)->resize_crop(400,400)->save($upload_path.$fileName,true);
                //$this->image_moo->load($upload_path.$fileName)->resize(400,400);

                if(file_exists(FCPATH.'assets/images/avatar/'.$userfile) && $userfile)
                unlink(FCPATH.'assets/images/avatar/'.$userfile);

                $this->db->update('tbl_main_user',array('photo'=> $fileName), array('user_id'=> $this->cms_user_id()));
            }
    ...
    public function user(){
        ...
        $session_nik = $this->cms_user_id();
        $crud->unset_jquery();
        $crud->add_action('Reset Password', base_url().'images/icn-lock.png', base_url().'images/icn-lock',base_url().'images/icn-lock.png',array($this,'reset_password_user'));
        ...
        $crud->unset_read();
        $crud->columns('user_id','user_name', 'email', 'real_name', 'active', 'groups','company','dUserUpdateTime');
        $crud->edit_fields('user_id','user_name', 'email', 'real_name', 'active', 'groups','groups_company');
        $crud->add_fields('user_id','user_name', 'email', 'password', 'real_name', 'active', 'groups','groups_company');
        $crud->field_type('active', 'true_false');
        $crud->required_fields('user_id','user_name', 'email', 'password', 'real_name', 'active');

        $crud->display_as('user_name', $this->cms_lang('User Name'))
            ->display_as('user_id', $this->cms_lang('NIK'))
            ...
            ->display_as('dUserUpdateTime', $this->cms_lang('Last Login'))
            ->display_as('password', $this->cms_lang('Password (md5)'));
        ...
        if ($this->cms_user_id()==1){
            ...
            $crud->where('user_id >=', 2);
            $crud->unset_delete();
            $crud->set_relation_n_n('groups', cms_table_name('main_group_user'), cms_table_name('main_group'), 'user_id', 'group_id', 'group_name', null, array('group_id !=' =>1));
        }
        */

        $crud->set_relation_n_n('groups', cms_table_name('main_group_user'), cms_table_name('main_group'), 'user_id', 'group_id', 'group_name', null, array('group_id !=' =>10));

        $crud->set_relation_n_n('groups_company', cms_table_name('group_company_user'), cms_table_name('company'), 'user_id', 'iCompanyId', 'cCompanyName', null, array('iCompanyId !=' =>0));    

        //$crud->set_relation_n_n('groups', cms_table_name('main_group_user'), cms_table_name('main_group'), 'user_id', 'group_id', 'group_name');
        //$crud->set_relation('company', $this->cms_complete_table_name('company'), '{Nama} [{Email}] - {NIK}',array('bStatus' =>1,'DeptID' => $DeptID));        
        $crud->callback_column('user_name',array($this,'_callback_column_user_name'));
        $crud->callback_column('company',array($this,'_callback_column_company'));

        //$crud->callback_column('password',array($this,'decode'));
    ...
    // add hyperlink
    public function reset_password_user($primary_key){

       
        $query  = mysql_query("SELECT * FROM tbl_main_user WHERE user_id='$primary_key'");
        $total  = mysql_num_rows($query);
        $data   = mysql_fetch_array($query);
        return "javascript:window.open('".base_url().'includes/mailer/reset_password/?user_id='.$primary_key."');void(0)";      
    ...
    // add hyperlink
    public function _callback_column_user_name($value, $row){  
        
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/edit/'.$row->user_id)."'>".$row->user_name."</a>";   
    ...
    public function _callback_column_company($value, $row){
        ...
        $query = $this->db->select('*')
            ...
            ->join('tbl_company', 'tbl_company.iCompanyId = tbl_profile.CompanyId')            
            ->where('tbl_profile.NIK', $row->user_id)
            ->get();
       
        foreach ($query->result() as $rown){
            if ($rown->cCompanyName == '' || empty($rown->cCompanyName) || is_null($rown->cCompanyName)){
                return '-';
            }
            else {
                return $rown->cCompanyCode;
            }
    ...
    public function group()
    {
        ...
        $crud->set_relation_n_n('navigations', cms_table_name('main_group_navigation'), cms_table_name('main_navigation'), 'group_id', 'navigation_id', 'navigation_name', null, array('navigation_name not like' => 'main%'));
    ...
    public function navigation($parent_id=NULL)
    {
        ...
        $crud->unique_fields('navigation_name','url');
        $crud->unset_read();
    ...
    // Jika Tidak mau memunculkan Menu (Complete Menu) Setting $no_complete_menu=TRUE
    // <img width="20" height="18" alt="Complete Menu" src="http://astapp02/hris2/assets/Menu4.png"  />

    public function widget_top_nav($caption = 'Menu', $first = TRUE, $no_complete_menu=TRUE, $no_quicklink=FALSE, $inverse = FALSE, $navigations = NULL){
        $result = '';
        ...
        if($first){
            ...
            if (!is_null($profile_user['photos_id'])){
                $session_img  = 'assets/uploads/files/'.$profile_user['photos_id'];
            }
            else {
            ...
            if (!empty($session_nik)){
                ...
                <ul class="nav pull-right navbar-nav">
                            <li class="dropdown">
                                ...
                                <img src="'.$this->set_user_avatar().'" style="height:34px; width:34px;" class="img-circle special-img"> {{ user_name }} <b class="caret"></b></a>
                                <ul class="dropdown-menu">
                                    <li><a href="'.site_url('blog/index/help').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-list-alt"></span> Help</a></li>
                                    <li><a href="'.site_url('blog/index/form_cuti_new').'" target="_blank"><span style="font-size:1.0em;" class="glyphicon glyphicon-list-alt"></span> Panduan Form Cuti</a></li>
                                    <li class="divider"></li>
                                    <li><a href="'.site_url('karyawan/frmProfile/').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-user"></span> My Profile</a></li>
                                    <!--<li><a href="'.site_url('karyawan/frmMyCV/').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-file"></span> My CV</a></li>-->
                                    <li><a href="'.site_url('employee/form_my_resume').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-file"></span> My Resume (CV) <span class="label label-success">New</span></a></li>
                                    '.$this->substitution_status_nav().$this->substitution_tasks_nav().'
                                    <li><a href="'.site_url('main/change_profile/').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-certificate"></span> Change Profile Login</a></li>

                                    <li class="divider"></li>
                                    <li><a href="'.site_url('main/logout/').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-off"></span> Logout</a></li>
                                    </ul>
                            </li>
                        </ul>';
            }
            ...
            // sisindokom
            if ($profile_user['company_id'] == $this->cms_get_config('cms_sisindokom_id')){
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logosis }}" style="max-height:35px; max-width:180px;" /></a>';
            }
            // jasnikom
            elseif ($profile_user['company_id'] == $this->cms_get_config('cms_jasnikom_id')){
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logojas }}" style="max-height:35px; max-width:180px;" /></a>';
            }

            elseif ($profile_user['company_id'] == $this->cms_get_config('cms_mediaindonusa_id')){
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logomediaindonusa }}" style="max-height:35px; max-width:180px;" /></a>';
            }            

            elseif ($profile_user['company_id'] == $this->cms_get_config('cms_mediatek_id')){
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logomediatek }}" style="max-height:35px; max-width:180px;" /></a>';
            }

            elseif ($profile_user['company_id'] == $this->cms_get_config('cms_mediaindonusa_id')){
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logodatacell }}" style="max-height:35px; max-width:180px;" /></a>';
            }

            elseif ($profile_user['company_id'] == $this->cms_get_config('cms_provis_id')){
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logoprovis }}" style="max-height:35px; max-width:180px;" /></a>';
            }

            else {
                $logo_img  = '<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logo }}" style="max-height:35px; max-width:180px;" /></a>';
            }
            ...
            <div class="navbar '.($inverse? 'navbar-inverse' : 'navbar-default').' navbar-fixed-top" role="navigation">
                ...
                <!--<header role="banner">
                    <a href="{{ site_url }}"><center><img id="logo-main" src="http://'.$_SERVER['SERVER_NAME'].'/hris2/logo2.png" style="max-height:35px; max-width:700px;"  alt="Human Resource Information System"></a>                
                </header>-->


                <div class="container">

                <!--<div><a href="{{ site_url }}"><center><img src ="http://'.$_SERVER['SERVER_NAME'].'/hris2/logo2.png" style="max-height:80px; max-width:700px;" /></center></a></div>-->
                    ...
                    <div class="navbar-header">
                        ...
                        <!--<a class="navbar-brand" href="#page-top">HRIS</a>-->

                        <!--<a class="navbar-brand" href="{{ site_url }}"><img src="http://astapp02/hris2/Logohris.png" style="max-height:35px; max-width:180px;" /></a>-->

                        <!--<a class="navbar-brand" href="{{ site_url }}"><img src ="{{ site_logo }}" style="max-height:35px; max-width:180px;" /></a>-->
                        '.$logo_img.'
    ...
    public function profile_user(){
        ...
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            $storeArray['company_id']      =  $row['CompanyId'];
            $storeArray['dept_id']         =  $row['DeptID'];
            $storeArray['sex_id']          =  $row['Sex'];
            $storeArray['status_diri']     =  $row['StatusDiri'];
            $storeArray['photos_id']       =  $row['Photos'];              
        }
    ...
    public function substitution_status_nav(){
        ...
        /*
        $query  = mysql_query('SELECT * FROM tbl_apv_group_approval WHERE NIK='.$this->cms_user_id());
        $total  = mysql_num_rows($query);
        $data   = mysql_fetch_array($query);

        $query2  = mysql_query('SELECT * FROM tbl_apv_hrd WHERE hrd_nik='.$this->cms_user_id());
        $total2  = mysql_num_rows($query2);
        $data2   = mysql_fetch_array($query2);
        */

        if ($total >0){
            $substitution_nav = '<li><a href="'.site_url('karyawan/frmSubstitution/').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-sort"></span> Substitution '.$this->current_substitution_status().'</a></li>';
        }
        else {
            $substitution_nav = '';
        }
    ...
    public function substitution_tasks_nav(){
        
        $query  = mysql_query('SELECT * FROM tbl_substitution_potential WHERE PotentialNIK='.$this->cms_user_id());
        $total  = mysql_num_rows($query);
        $data   = mysql_fetch_array($query);
        if ($total >0){
            $substitution_nav = '<li><a href="'.site_url('karyawan/frmTasks/').'"><span style="font-size:1.0em;" class="glyphicon glyphicon-bell"></span> Tasks '.$this->form_tasks_total($nik=$this->cms_user_id()).'</a></li>';
        }
        else {
            $substitution_nav = '';
        }
    ...
    public function current_substitution_status(){
        $this->load->model('form_substitution_model');
        $data = $this->form_substitution_model->current_substitution_status($nik=$this->cms_user_id());
        return $data;
    }

    public function form_tasks_total($nik){
        $this->load->model('form_substitution_model');
        $data = $this->form_substitution_model->all_potential_user_form($nik);
        if ($data > 0){
        ...
        foreach($quicklinks as $quicklink){
            ...
            if(count($quicklink['child'])==0){
                ...
                if($first){
                    ...
                    $html.= '<a class="dropdown-toggle" data-toggle="dropdown" href="javascript:void(0);">'.
    ...
    public function set_user_avatar(){
        ...
        $SQL   = "SELECT user_name, email, real_name, photo FROM tbl_main_user WHERE user_id =".$this->cms_user_id();
        $query = $this->db->query($SQL);
        $row   = $query->row();

        if (!is_null($row->photo) && file_exists(FCPATH.'assets/images/avatar/'.$row->photo)){
            $session_img  = site_url('assets/images/avatar/'.$row->photo);
        }
        else {
            $session_img  = site_url('assets/images/avatar/default.png');
        }
