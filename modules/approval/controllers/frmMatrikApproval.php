 ...
 */
class frmMatrikApproval extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('apv_matrik_approval'));
        //$crud->order_by('Tanggal', 'ASC');
        // primary key
        $crud->set_primary_key('MatId');

        // set subject
        $crud->set_subject('Matrik Approval');
        $crud->change_field_type('MatProses', 'integer');

        // displayed columns on list
		$crud->columns('MatCode','MatProses','MatName','MatAction','deptID','MatStatus','companyID');
        $crud->edit_fields('MatCode','MatProses','MatName','MatAction','MatStatus','companyID','divisionID','deptID');
		$crud->add_fields('MatCode','MatProses','MatName','MatAction','MatStatus','companyID','divisionID','deptID');
        ...
        $crud->display_as('MatName','Name');
        $crud->display_as('MatCode','Approval');
        ...
        $crud->display_as('divisionID','Divisi');
        $crud->display_as('deptID','Group');      
				
		
		// $crud->required_fields( $field1, $field2, $field3, ... );
		$crud->required_fields('MatCode','MatProses','MatName','MatAction','MatStatus','companyID');
       		
		//$crud->unique_fields( $field1, $field2, $field3, ... );
		//$crud->unique_fields('NIK');
       		

		$crud->set_relation('MatCode', $this->cms_complete_table_name('apv_matrik_code'), 'MatCodeName');
        $crud->set_relation('MatAction', $this->cms_complete_table_name('apv_matrik_status'), 'mStatusName');
       ...
       // $crud->set_relation('JabatanID', $this->cms_complete_table_name('jabatan'), 'NamaJabatan');
        ...
        $crud->callback_column('MatName',array($this,'_callback_column_MatName'));
    ...
    public function _example_output($output = null)
    {
        ...
        $this->view($this->cms_module_path().'/frmMatrikApproval_view', $output,
        $this->cms_complete_navigation_name('frmMatrikApproval'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/approval/frmMatrikApproval/get_states/', site_url().'/approval/frmMatrikApproval/get_cities/'),
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('apv_matrik_approval'),array('MatId'=>$id));
    ...
    public function _callback_column_MatName($value, $row){    
         
    //$Date2 = date('d-M-Y', strtotime($row->Tanggal));
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/index/edit/'.$row->MatId)."'>".$row->MatName."</a>";
    ...
    public function empty_state_dropdown_select()
    {
        ...
        if(isset($listingID) && $state == "edit") {
            ...
            $this->db->select('companyID, divisionID, deptID, MatId')
                     ->from('tbl_apv_matrik_approval')
                     ->where('MatId', $listingID);
            $db = $this->db->get();
            $row = $db->row(0);
