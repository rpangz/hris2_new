...
class frmGroupApproval extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        if ($state !='edit' AND $state != 'add' AND $state !='read'){
            $crud->set_theme('flexigrid');
             $crud->set_relation('NIK', $this->cms_complete_table_name('profile'), '{NIK}');
            ...
            //$crud->unset_edit();
        	 ...
        	 $crud->set_relation('NIK', $this->cms_complete_table_name('profile'), '{Nama} - {NIK}');
        ...
        */
        ...
        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4))){
            $crud->where('tbl_apv_group_approval.deptID', $this->uri->segment(4));
            $crud->order_by('iGroupApprovalListId', 'ASC');

        }
        else {
            $crud->where('tbl_apv_group_approval.deptID >=', 0);
            $crud->order_by('deptID','iGroupApprovalListId','ASC');
        ...
        $crud->set_table($this->cms_complete_table_name('apv_group_approval'));
        ...
        //$crud->order_by('iGroupApprovalListId','ASC');
        // primary key
        $crud->set_primary_key('iGroupApprovalId');

        $crud->set_subject('Group Approval');
        ...
        $crud->display_as('form_kasbon','Form Kasbon');
        $crud->display_as('form_kpi','Form KPI');
        $crud->display_as('iUnitID','Unit');
    ...
    public function _example_output($output = null)
    {
        ...
        $this->view($this->cms_module_path().'/frmGroupApproval_view', $output,
        $this->cms_complete_navigation_name('frmGroupApproval'));    
    }
