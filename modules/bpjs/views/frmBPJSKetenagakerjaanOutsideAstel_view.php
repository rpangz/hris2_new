...
<div class="row">
  ...
  echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/bpjs/frmBPJSKetenagakerjaanOutsideAstel/index/0' >ALL Company</option>";
    $sql = mysql_query("SELECT * FROM tbl_company WHERE iCompanyId !='1' AND iCompanyId !='2' ORDER BY iCompanyId ASC");           
     while ($data = mysql_fetch_array($sql)){

  if($this->uri->segment('4')==$data['iCompanyId']){
     echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/bpjs/frmBPJSKetenagakerjaanOutsideAstel/index/$data[iCompanyId]' SELECTED>$data[cCompanyName]</option>";  
      
    }
      else {
      echo "<option value='http://".$_SERVER['SERVER_NAME']."/hris2/bpjs/frmBPJSKetenagakerjaanOutsideAstel/index/$data[iCompanyId]'>$data[cCompanyName]</option>";
    }
      }  
