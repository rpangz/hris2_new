 ...
 */
class frmYear extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('year'));
        //$crud->order_by('Tanggal', 'ASC');
        // primary key
        $crud->set_primary_key('TahunId');

        // set subject
        $crud->set_subject('Year');

        // displayed columns on list
		$crud->columns('NamaTahun','Status');

        $crud->edit_fields('NamaTahun','Status');
		$crud->add_fields('NamaTahun','Status');
        ...
        $crud->display_as('NamaTahun','Name');
        $crud->display_as('Status','Status');
        //$crud->display_as('IsWorking','Status');  
				
		
		// $crud->required_fields( $field1, $field2, $field3, ... );
		$crud->required_fields('NamaTahun','Status');
       		
		//$crud->unique_fields( $field1, $field2, $field3, ... );
		$crud->unique_fields('NamaTahun');
       ...
       // $crud->set_relation('JabatanID', $this->cms_complete_table_name('jabatan'), 'NamaJabatan');
        ...
        $crud->callback_column('NamaTahun',array($this,'_callback_column_NamaTahun'));
    ...
    public function index(){
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/frmYear_view', $output,
            $this->cms_complete_navigation_name('frmYear'));
    }
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('year'),array('TahunId'=>$id));
    ...
    public function _callback_column_NamaTahun($value, $row){    
         
    //$Date2 = date('d-M-Y', strtotime($row->Tanggal));
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/index/edit/'.$row->TahunId)."'>".$row->NamaTahun."</a>";
