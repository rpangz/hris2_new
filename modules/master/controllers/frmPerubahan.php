 ...
 */
class frmPerubahan extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('jenisperubahan'));
        //$crud->order_by('Tanggal', 'ASC');
        // primary key
        $crud->set_primary_key('PerId');

        // set subject
        $crud->set_subject('Jenis Perubahan');

        // displayed columns on list
		$crud->columns('PerName','PerCode');

        $crud->edit_fields('PerName','PerCode');
		$crud->add_fields('PerName','PerCode');
        ...
        $crud->display_as('PerName','Name');
        $crud->display_as('PerCode','Code');
        //$crud->display_as('IsWorking','Status');  
				
		
		// $crud->required_fields( $field1, $field2, $field3, ... );
		$crud->required_fields('PerName','PerCode');
       ...
       // $crud->set_relation('JabatanID', $this->cms_complete_table_name('jabatan'), 'NamaJabatan');
        ...
        $crud->callback_column('PerName',array($this,'_callback_column_PerName'));
    ...
    public function index(){
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/frmPerubahan_view', $output,
            $this->cms_complete_navigation_name('frmPerubahan'));
    }
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('jenisperubahan'),array('PerId'=>$id));
    ...
    public function _callback_column_PerName($value, $row){    
         
    //$Date2 = date('d-M-Y', strtotime($row->Tanggal));
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/index/edit/'.$row->PerId)."'>".$row->PerName."</a>";
