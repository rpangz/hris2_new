 ...
 */
class frmJenisIjin extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('jenisijin'));
        //$crud->order_by('Tanggal', 'ASC');
        // primary key
        $crud->set_primary_key('JenisIjinId');

        // set subject
        $crud->set_subject('Jenis Ijin');

        // displayed columns on list
		$crud->columns('JenisIjinName','JenisIjinCode');
        $crud->edit_fields('JenisIjinName','JenisIjinCode');
		$crud->add_fields('JenisIjinName','JenisIjinCode');
        ...
        $crud->display_as('JenisIjinName','Name');
        $crud->display_as('JenisIjinCode','Code');
        //$crud->display_as('IsWorking','Status');  
				
		
		// $crud->required_fields( $field1, $field2, $field3, ... );
		$crud->required_fields('JenisIjinName','JenisIjinCode');
       ...
       // $crud->set_relation('JabatanID', $this->cms_complete_table_name('jabatan'), 'NamaJabatan');
        ...
        $crud->callback_column('JenisIjinName',array($this,'_callback_column_JenisIjinName'));
    ...
    public function index(){
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        $output = $crud->render();
        $this->view($this->cms_module_path().'/frmJenisIjin_view', $output,
            $this->cms_complete_navigation_name('frmJenisIjin'));
    }
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('jenisijin'),array('JenisIjinId'=>$id));
    ...
    public function _callback_column_JenisIjinName($value, $row){    
         
    //$Date2 = date('d-M-Y', strtotime($row->Tanggal));
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/index/edit/'.$row->JenisIjinId)."'>".$row->JenisIjinName."</a>";
