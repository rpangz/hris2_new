 ...
 */
class frmFaskesGigi extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('faskesgigi'));
        //$crud->ORDER_BY('iDeptDivID','ASC');

        $list = $this->uri->segment(4);
        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4)) && isset($list)){
            $crud->where('tbl_faskesgigi.KDPROP', $this->uri->segment(4));
        }       
        else {
            $crud->where('tbl_faskesgigi.KDPROP >=', 0);          
        }

        // primary key
        $crud->set_primary_key('KDFaskesGigi');

        // set subject
        $crud->set_subject('Faskes Gigi');

        // displayed columns on list
        $crud->columns('KDFaskesGigi','NMFaskesGigi','KDDATI2','KDPROP');
        // displayed columns on edit operation
        $crud->edit_fields('KDPROP','KDDATI2','KDFaskesGigi','NMFaskesGigi');
        // displayed columns on add operation
        $crud->add_fields('KDPROP','KDDATI2','KDFaskesGigi','NMFaskesGigi');
        ...
        $crud->display_as('NMFaskesGigi','Nama Faskes Gigi');
        $crud->display_as('KDFaskesGigi','Kode Faskes Gigi');
        ...
        $crud->required_fields('KDPROP','KDDATI2','NMFaskesGigi','KDFaskesGigi');
		...
		$crud->unique_fields('KDFaskesGigi');
       ...
       // $crud->callback_column('KDPROP',array($this, '_callback_column_KDPROP'));
        $crud->callback_column('NMFaskesGigi',array($this,'_callback_edit_url'));
    ...
    public function _example_output($output = null)
    {
        ...
        $this->view($this->cms_module_path().'/frmFaskesGigi_view', $output,
        $this->cms_complete_navigation_name('frmFaskesGigi'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/master/frmFaskesGigi/get_states/', site_url().'/master/frmFaskesGigi/get_cities/'),
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('faskesgigi'),array('KDFaskesGigi'=>$id));
    ...
    public function _callback_edit_url($value, $row){    
        ...
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/index/edit/'.$row->KDFaskesGigi)."'>$value</a>";
    ...
    {
        //CREATE THE EMPTY SELECT STRING
        $empty_select = '<select name="KDFaskesGigi" class="chosen-select" data-placeholder="Select Faskes Gigi" style="width: 300px; display: none;">';
        $empty_select_closed = '</select>';
        //GET THE ID OF THE LISTING USING URI
        $listingID = $this->uri->segment(5);
        ...
        if(isset($listingID) && $state == "edit") {
            ...
            //APPEND THE OPTION FIELDS WITH VALUES FROM THE STATES PER THE COUNTRY ID
            foreach($db->result() as $row):
                if($row->KDFaskesGigi == $KDFaskesGigi) {
                    $empty_select .= '<option value="'.$row->KDFaskesGigi.'" selected="selected">'.$row->NMFaskesGigi.'</option>';
                } else {
                    $empty_select .= '<option value="'.$row->KDFaskesGigi.'">'.$row->NMFaskesGigi.'</option>';
                }
            endforeach;
    ...
    public function get_cities()
    {
        ...
        $array = array();
        foreach($db->result() as $row):
            $array[] = array("value" => $row->KDFaskesGigi, "property" => $row->NMFaskesGigi);
        endforeach;
