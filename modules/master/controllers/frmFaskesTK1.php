 ...
 */
class frmFaskesTK1 extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        $crud->unset_jquery();
        $crud->unset_read();
        $crud->unset_texteditor('AlamatFaskes');
        // $crud->unset_add();
        // $crud->unset_edit();
        // $crud->unset_delete();
        // $crud->unset_list();
        ...
        */
        ...
        // table name
        $crud->set_table($this->cms_complete_table_name('faskesumum'));
        //$crud->ORDER_BY('iDeptDivID','ASC');

        $list = $this->uri->segment(4);
        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4)) && isset($list)){
            $crud->where('tbl_faskesumum.KDPROP', $this->uri->segment(4));
        }       
        else {
            $crud->where('tbl_faskesumum.KDPROP >=', 0);          
        }

        // primary key
        $crud->set_primary_key('KDFaskes');

        // set subject
        $crud->set_subject('Faskes Tk.1');

        // displayed columns on list
        $crud->columns('KDFaskes','NMFaskes','TipeFaskes','AlamatFaskes','KDDATI2','KDPROP');
        // displayed columns on edit operation
        $crud->edit_fields('KDPROP','KDDATI2','KDFaskes','NMFaskes','TipeFaskes','AlamatFaskes','RT','RW','KodePos_Faskes','NoTelp_Faskes');
        // displayed columns on add operation
        $crud->add_fields('KDPROP','KDDATI2','KDFaskes','NMFaskes','TipeFaskes','AlamatFaskes','RT','RW','KodePos_Faskes','NoTelp_Faskes');
        ...
        $crud->display_as('NMFaskes','Nama Faskes Tk.1');
        $crud->display_as('KDFaskes','Kode Faskes');
        $crud->display_as('TipeFaskes','Type Faskes');
        $crud->display_as('AlamatFaskes','Alamat Faskes');
        $crud->display_as('KodePos_Faskes','Kode Pos');
        $crud->display_as('NoTelp_Faskes','No. Telp');        
        ...
        $crud->required_fields('KDPROP','KDDATI2','NMFaskes','KDFaskes');
       ...
       // $crud->callback_column('KDPROP',array($this, '_callback_column_KDPROP'));
        $crud->callback_column('NMFaskes',array($this,'_callback_edit_url'));
        ...
        $crud->set_relation('TipeFaskes', $this->cms_complete_table_name('tipe_faskes'), 'nm_tipe');
    ...
    public function _example_output($output = null)
    {
        ...
        $this->view($this->cms_module_path().'/frmFaskesTK1_view', $output,
        $this->cms_complete_navigation_name('frmFaskesTK1'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/master/frmFaskesTK1/get_states/', site_url().'/master/frmFaskesTK1/get_cities/'),
        ...
        if(!$crud->unset_delete){
            ...
            foreach($id_list as $id){
                if($this->_before_delete($id)){
                    $this->db->delete($this->cms_complete_table_name('faskesumum'),array('KDFaskes'=>$id));
    ...
    public function _callback_edit_url($value, $row){    
        ...
        return "<a href='".site_url($this->cms_module_path().'/'.$this->uri->segment('2').'/index/edit/'.$row->KDFaskes)."'>$value</a>";
    ...
    {
        //CREATE THE EMPTY SELECT STRING
        $empty_select = '<select name="KDFaskes" class="chosen-select" data-placeholder="Select Faskes Tk.1" style="width: 300px; display: none;">';
        $empty_select_closed = '</select>';
        //GET THE ID OF THE LISTING USING URI
        $listingID = $this->uri->segment(5);
        ...
        if(isset($listingID) && $state == "edit") {
            ...
            foreach($db->result() as $row):
                if($row->KDFaskes == $KDFaskes) {
                    $empty_select .= '<option value="'.$row->KDFaskes.'" selected="selected">'.$row->NMFaskes.'</option>';
                } else {
                    $empty_select .= '<option value="'.$row->KDFaskes.'">'.$row->NMFaskes.'</option>';
                }
            endforeach;
    ...
    public function get_cities()
    {
        ...
        $array = array();
        foreach($db->result() as $row):
            $array[] = array("value" => $row->KDFaskes, "property" => $row->NMFaskes);
        endforeach;
