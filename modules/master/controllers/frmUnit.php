 ...
 */
class frmUnit extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        */
		...
		$crud->set_table($this->cms_complete_table_name('unit'));

        $list = $this->uri->segment(4);
        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4)) && isset($list)){
            $crud->where('tbl_unit.companyID', $this->uri->segment(4));
        }       
        else {
            $crud->where('tbl_unit.companyID >=', 0);          
        }
        ...
        $crud->order_by('tbl_unit.companyID','ASC');
        $crud->order_by('tbl_unit.divisionID','ASC');
        $crud->order_by('DeptID','ASC');
        // primary key
        $crud->set_primary_key('unitID');
        ...
        $crud->columns('unitID','NamaUnit','deptID','divisionID','companyID');
        $crud->edit_fields('companyID','divisionID','deptID','NamaUnit');
        $crud->add_fields('companyID','divisionID','deptID','NamaUnit');
        ...
        $crud->display_as('unitID','Unit ID');

        $crud->required_fields('deptID','NamaUnit');
    ...
    public function _example_output($output = null){
        ...
        $this->view($this->cms_module_path().'/frmUnit_view', $output,
        $this->cms_complete_navigation_name('frmUnit'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/master/frmUnit/get_states/', site_url().'/master/frmUnit/get_cities/'),
    ...
    public function _callback_webpage_url($value, $row){        
        ...
        return "<a href='".site_url($this->cms_module_path().'/frmUnit/index/edit/'.$row->unitID)."'>$value</a>";
