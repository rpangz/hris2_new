...
echo"<table>
  <tr>
    ...
    <option value='http://$_SERVER[SERVER_NAME]/hris2/master/frmFaculty/index/0'>---ALL---</option>";
    $sql = mysql_query("SELECT * FROM tbl_school_level ORDER BY SchoolLevelId ASC");                     
    while ($data = mysql_fetch_array($sql)){

    if($this->uri->segment('4')==$data['SchoolLevelId']){  
      echo "<option value='http://$_SERVER[SERVER_NAME]/hris2/master/frmFaculty/index/$data[SchoolLevelId]' SELECTED>$data[SchoolLevelName]</option>";
    }
    else {
      echo "<option value='http://$_SERVER[SERVER_NAME]/hris2/master/frmFaculty/index/$data[SchoolLevelId]'>$data[SchoolLevelName]</option>";
    }
