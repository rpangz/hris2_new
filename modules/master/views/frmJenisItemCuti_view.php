...
echo"<table>
  <tr>
    ...
    $sql = mysql_query("SELECT * FROM tbl_jeniscuti ORDER BY id ASC");                     
    while ($data = mysql_fetch_array($sql)){

    if($this->uri->segment('4')==$data['id']){  
      echo "<option value='http://$_SERVER[SERVER_NAME]/hris2/master/frmJenisItemCuti/index/$data[id]' SELECTED>$data[JenisCutiName]</option>";
    }
    else {
      echo "<option value='http://$_SERVER[SERVER_NAME]/hris2/master/frmJenisItemCuti/index/$data[id]'>$data[JenisCutiName]</option>";
    }
