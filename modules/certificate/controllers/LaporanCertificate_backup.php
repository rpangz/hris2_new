...
/*
        ...
        if($company!=0 && $company!="") {$conscript = "AND CompanyID = '".$company."'"; }
        $jenissertifikasi = $this->uri->segment(5);
...
*/
        ...
        if ($jenissertifikasi == "NON"){
            $where=" (CertItem != 3 AND CertItem != 4) 
                     AND (CertValidFinish BETWEEN STR_TO_DATE('".$tglawal."','%d-%m-%Y') AND STR_TO_DATE('".$tglakhir."','%d-%m-%Y')) 
                     AND CertStatus=1 AND bStatus=1 ".$conscript;            
        } elseif ($jenissertifikasi == "CCIE") {
            $where=" (CertItem = 3 OR CertItem = 4) 
                     AND (CertValidFinish BETWEEN STR_TO_DATE('".$tglawal."','%d-%m-%Y') AND STR_TO_DATE('".$tglakhir."','%d-%m-%Y')) 
                     AND CertStatus=1 AND bStatus=1 ".$conscript;
        } else {
            $where=" (CertValidFinish BETWEEN STR_TO_DATE('".$tglawal."','%d-%m-%Y') AND STR_TO_DATE('".$tglakhir."','%d-%m-%Y'))
                     AND CertStatus=1 AND bStatus=1 ";
        }
