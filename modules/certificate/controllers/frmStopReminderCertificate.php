...
class frmStopReminderCertificate extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        $crud->set_table('tbl_profile_certification');
        $crud->set_relation('CertNIK','tbl_profile','{Nama} - {NIK}');
        $crud->columns('CertId','CertNIK','CertName','CertValidFinish');
        if ($this->uri->segment(4) == "NON"){
            $where=" (CertItem != 3 AND CertItem != 4) AND CertValidFinish<=DATE_ADD(now(), INTERVAL 3 MONTH) AND
                     CertStatus=1 AND bStatus=1 ";            
        } elseif ($this->uri->segment(4) == "CCIE") {
            $where=" (CertItem = 3 OR CertItem = 4) AND CertValidFinish<=DATE_ADD(now(), INTERVAL 6 MONTH) AND
                     CertStatus=1 AND bStatus=1 ";
        } else {
            $where=" CertValidFinish<=DATE_ADD(now(), INTERVAL 6 MONTH) AND
                     CertStatus=1 AND bStatus=1 ";    
        }
        $crud->where($where);  
...
/*
        $crud->columns('iGroupApprovalListId','NIK','NIKKI','iUnitID','deptID','divisionID','companyID','form_cuti','form_ijin','form_my_cv','form_kasbon','form_kpi','potential','active_sub');
        $crud->edit_fields('companyID','divisionID','deptID','iUnitID','iGroupApprovalListId','NIK','form_cuti','form_ijin','form_my_cv','form_kasbon','form_kpi','potential','active_sub');
        $crud->add_fields('companyID','divisionID','deptID','iUnitID','iGroupApprovalListId','NIK','form_cuti','form_ijin','form_my_cv','form_kasbon','form_kpi');
 */         
        $crud->display_as('CertId','CERTIFICATE ID');   
        $crud->display_as('CertNIK','NAMA - NIK');   
        $crud->display_as('nama','NAME');   
        $crud->display_as('CertName','CERTIFICATE NAME');   
        $crud->display_as('CertValidFinish','CERTIFICATE VALID');   
        $crud->display_as('keterangan','CATEGORY');   

        $crud->add_action('Delete Certificate', base_url().'images/icon_delete.png', base_url().'images/icon_delete',base_url().'images/icon_delete.png',array($this,'delete_certificate'));
        $crud->add_action('Stop Scheduler', base_url().'images/icon_stop.png', base_url().'images/icon_stop',base_url().'images/icon_stop.png',array($this,'stop_scheduler'));
...
/*
    public function stop_scheduler($primary_key){
        return "javascript:window.open('".base_url()."includes/mailer/frmStopReminderCertificate/?certid=".$primary_key."');void(0);location.reload();";
    }
*/
    public function stop_scheduler($primary_key){
        return "javascript:window.location = '".base_url()."includes/mailer/frmStopReminderCertificate/?certid=".$primary_key."&&userid=".$this->cms_user_id()."'";
    }

    public function delete_certificate($primary_key){
        return "javascript:window.location = '".base_url()."includes/mailer/frmStopReminderCertificate/deletecertificate.php?certid=".$primary_key."&&userid=".$this->cms_user_id()."'";
    }


/*
    public function stop_scheduler($primary_key){
        return "javascript:loadprompt(".$primary_key.")";
    }
*/ 
    public function _example_output($output = null)
    {
        //$this->view($this->cms_module_path().'modules/customers', $output);
        $this->view($this->cms_module_path().'/frmStopReminderCertificate_view', $output,
        $this->cms_complete_navigation_name('frmStopReminderCertificate'));    
    }
