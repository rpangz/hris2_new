...
class frmListCutiUtang extends CMS_Priv_Strict_Controller {
    ...
    private function make_crud(){
        ...
        $company_id = $this->cms_get_config('cms_mediaindonusa_id');
      ...
      //$crud->set_relation('FormCutiNIK', $this->cms_complete_table_name('profile'), 'Nama',array('NIK >' => $NIKKI, 'CompanyId' => $company_id));
        ...
        $crud->where('HakCutiId', 0);

        if ($this->uri->segment(4) !=0 && !is_null($this->uri->segment(4)) && isset($list) && $list <=2){
        ...
        $crud->set_subject('Form Cuti Utang '.$nmc);

        $crud->unset_add_fields('CreatedBy','CreatedTime','Tgl1','Tgl2','Tgl3','Apv1','Apv2','Apv3','StatusForm','ApvLevel','CutiUtangId');
       ...
       $crud->field_type('CutiUtangId', 'hidden',1);
       ...
       // caption of each columns      
        ...
        $crud->display_as('Detail','Cuti');
        $crud->display_as('TglMasuk','Masuk Kerja');
...
*/
        $crud->set_relation('NIK3', $this->cms_complete_table_name('apv_hrd'), '{hrd_name}',array('hrd_status' => 1,'hrd_company' => $company_id,'hrd_modules'=>'form_cuti'));

        if ($state=='read'){
            $crud->edit_fields('FormCutiNIK','JenisCuti','Keperluan','Alamat','NIKPengganti','NoTelpon','Detail','TglMasuk','NIK1','NIK2','NIK3');
            $crud->set_relation('NIKPengganti', $this->cms_complete_table_name('profile'), '{Nama}');
        }
        else{
            $crud->edit_fields('StatusForm','Alasan');
            $crud->set_relation('NIKPengganti', $this->cms_complete_table_name('profile'), '{Nama}',array('bStatus' =>1,'DeptID' => $DeptID));
        }
        ...
        //$crud->lang['SendMail_inactive']              = 'No';
        //$crud->lang['SendMail_active']                = 'Yes';

     

        //$crud->change_field_type('SendMail', 'true_false');

        //$crud->callback_add_field('SendMail',array($this,'_callback_add_field_SendMail'));
        ...
        //$crud->callback_column('FormCutiNIK',array($this,'_callback_column_FormCutiNIK'));
        ...
        if (!is_null($this->input->get('hc')) AND $this->uri->segment(4) !=3){            
            $crud->add_fields('JenisCuti','FormCutiNIK','HakCutiId2','Keperluan','Alamat','NIKPengganti','NoTelpon','Detail','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CutiUtangId','CreatedBy','CreatedTime','SendMail','companyID');
            $crud->callback_add_field('HakCutiId2', array($this, '_callback_add_field_HakCutiId2'));
        
        }
        elseif (!is_null($this->input->get('hc')) AND $this->uri->segment(4) ==3){
            $crud->add_fields('JenisCuti','FormCutiNIK','HakCutiId2','Keperluan','Alamat','NIKPengganti','NoTelpon','TglActive1','TglActive2','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CutiUtangId','CreatedBy','CreatedTime','SendMail','companyID');
            $crud->callback_add_field('HakCutiId2', array($this, '_callback_add_field_HakCutiId2'));
            ...
            $crud->add_fields('JenisCuti','FormCutiNIK','HakCutiId','Keperluan','Alamat','NIKPengganti','NoTelpon','Detail','TglMasuk','NIK1','Apv1','NIK2','Apv2','NIK3','Apv3','Tgl1','Tgl2','Tgl3','StatusForm','ApvLevel','CutiUtangId','CreatedBy','CreatedTime','SendMail','companyID');
        ...
        //$crud->add_action('Print', 'http://astapp02/hris2/includes/images/print1.png');
        ...
        //$crud->add_action('Print', '', '','ui-icon-image',array($this,'_callback_column_Print'));
    ...
    public function _example_output($output = null){
        ...
        $this->view($this->cms_module_path().'/frmListCutiUtang_view', $output,
        $this->cms_complete_navigation_name('frmListCutiUtang'));    
    }
    ...
    public function index(){
        ...
        $dd_data = array(
                ...
                'dd_url' => array('', site_url().'/'.$this->cms_module_path().'/'.$this->uri->segment('2').'/get_states/', site_url().'/'.$this->cms_module_path().'/'.$this->uri->segment('2').'/get_cities/'),
    ...
    */
    ...
    public function _after_insert($post_array, $primary_key){
        ...
        //$dt       = mysql_fetch_array(mysql_query("SELECT * FROM tbl_formcuti WHERE FormCutiNIK=".$session_nik." ORDER BY CutiId DESC LIMIT 1"));
        //$ID       = $dt['CutiId'];

        $ID       = $primary_key;
        $SendMail = $post_array['SendMail'];

        $TglMasuk    = date('Y-m-d', strtotime($post_array['TglMasuk']));
        ...
        if(($num_row ==0 || $total ==0) && $SendMail ==1){
            ...
            include "http://".$_SERVER['SERVER_NAME']."/hris2/includes/mailer/frmListCutiUtang/SendMail.php?id=".$primary_key;
        ...
        elseif(($num_row ==0 || $total ==0) && $SendMail ==0){
            ...
            echo "<script language='javascript'>alert('Data Sudah Disimpan...');history.go(-1);</script>";
    ...
    public function _after_update($post_array, $primary_key){
        ...
        include "http://$_SERVER[SERVER_NAME]/hris2/includes/mailer/frmListCutiUtang/SendMailVoid.php?id=$ID";
        echo "<script language='javascript'>alert('Data Sudah Disimpan dan Email sudah dikirim...');history.go(-1);</script>";
    ...
    public function _callback_add_field_HakCutiId2(){
       ...
       $query = $this->db->select('*')
        ...
        $SQL1 = mysql_query("SELECT * FROM tbl_hakcuti WHERE NIK='0' AND HakId='0' ORDER BY HakId ASC");
    ...
    $hh = mysql_query("SELECT * FROM `tbl_formcuti` INNER JOIN tbl_formcutidetail ON tbl_formcuti.CutiId=tbl_formcutidetail.CutiId 
        WHERE tbl_formcuti.HakCutiId='$row1[HakId]' AND tbl_formcuti.StatusForm='A' AND tbl_formcuti.FormCutiNIK='$FormCutiNIK' AND CutiUtangId=1");
        ...
        $value = $grand;
            ...
            $dta      = mysql_fetch_array(mysql_query('SELECT count(FormCutiNIK) AS QtyPakai FROM tbl_formcuti INNER JOIN 
                                                    ...
                                                    WHERE tbl_formcuti.FormCutiNIK="'.$FormCutiNIK.'" AND HakCutiId="0" AND StatusForm="A" '));
            ...
            $empty_select = $Periode1.''.$Periode2.' Sisa Cuti '.$value." Days <input type='hidden' maxlength='50' value='".$row->HakId."' name='HakCutiId'>";
    ...
    public function empty_state_dropdown_select(){
        ...
        if(isset($listingID) && $state == "edit"){
            ...
            foreach($db->result() as $row):
                ...
                $dta = mysql_fetch_array(mysql_query('SELECT count(FormCutiNIK) AS QtyPakai FROM tbl_formcuti INNER JOIN 
                                                    tbl_formcutidetail ON tbl_formcuti.CutiId = tbl_formcutidetail.CutiId 
                                                    WHERE tbl_formcuti.FormCutiNIK="'.$FormCutiNIK.'" AND HakCutiId=0 AND CutiUtangId=1 
                                                    AND StatusForm="A" '));
            ...
            elseif ($today > $row->PeriodeExt && !is_null($row->PeriodeExt)){
                ...
                if($row->HakId == $HakCutiId) {
                    $empty_select .= '<option value="'.$link.'" selected="selected">'.$Periode1.''.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                } else {
                    $empty_select .= '<option value="'.$link.'">'.$Periode1.''.$Periode2.'  [sisa : '.$Sisa.' hari]</option>';
                }
            endforeach;
    ...
    public function get_states(){
        ...
        $jc = mysql_fetch_array(mysql_query('SELECT * FROM tbl_jeniscuti WHERE id="'.$JenisCuti.'"'));
    ...
    $hh = mysql_query("SELECT * FROM `tbl_formcuti` INNER JOIN tbl_formcutidetail ON tbl_formcuti.CutiId=tbl_formcutidetail.CutiId 
        ...
        $value = $grand;
            ...
            $dta      = mysql_fetch_array(mysql_query('SELECT count(FormCutiNIK) AS QtyPakai FROM tbl_formcuti INNER JOIN 
                                                    tbl_formcutidetail ON tbl_formcuti.CutiId = tbl_formcutidetail.CutiId 
                                                    WHERE tbl_formcuti.FormCutiNIK="'.$FormCutiNIK.'" AND HakCutiId=0 AND CutiUtangId=1 AND StatusForm="A"'));
           ...
           if (is_null($row->PeriodeExt) OR $row->PeriodeExt ==""){
            ...
            $array[] = array("value" => '?nik='.$FormCutiNIK.'&hc='.$HakId ,"property" => $Periode1.''.$Periode2.' Sisa Cuti : '.$value.' Days');
        endforeach;
    ...
    public function _callback_column_Print($value, $row){
            ...
            //return "javascript:openBlank('".site_url('includes/export/frmListCuti/Export2Html2.php') . '?id=' . $row->CutiId . "')";
