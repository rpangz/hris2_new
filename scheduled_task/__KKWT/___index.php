...
echo "today= $today | target=$target | begin=$Begin | interval= $interval | input= $input | repeat= $Repeat | $init";
...
while ($dad = mysql_fetch_array($rows_data)){
	...
	$query 	="SELECT * FROM tbl_kkwt WHERE KKWTId IN (SELECT MAX( KKWTId ) FROM tbl_kkwt 
			  WHERE active_id =1 AND KKWT_Date2 <= '$Begin' 
...
while ($data = mysql_fetch_array($sql)){	
	...
	$konten[$NIK]	="<tr>
								...
								<td><div align='left'>$data[NIK]</div></td>
								<td><div align='left'>$row[Name]</div></td>
								<td><div align='left'>$data[KKWTId]</div></td>
								<td><div align='left'>$data[KKWT_Name]</div></td>
								<td><div align='left'>$Date2</div></td>								
							</tr>";
	...
	<p><font color=#FF0000 size=-1>Do not reply this email</font></p>	
	</body>
	</html>";	
