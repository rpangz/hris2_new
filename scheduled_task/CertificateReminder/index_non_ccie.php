...
// Send Mail to User
$sql        = mysql_query('SELECT *,'.select_value_sub().' FROM tbl_profile_certification INNER JOIN tbl_profile ON CertNIK=NIK WHERE '.where_value().' GROUP BY CertNIK '.having_value().' ORDER BY CertNIK ASC');
...
while ($data = mysql_fetch_array($sql)){
    ...
    SendMail($nikki,$email,$nama,$where_nik,$where_valid,$where_dept,$company_id,$notes='Silahkan Diperiksa');    
...
$sql1        = mysql_query('SELECT *,'.select_value_sub().' FROM tbl_apv_hrd INNER JOIN 
                            ...
                            WHERE '.where_value().' AND bStatus=1 AND hrd_modules="certificate_reminder" AND hrd_status=1 GROUP BY hrd_company,hrd_nik '.having_value());
...
while ($data1 = mysql_fetch_array($sql1)){
    ...
    SendMail($nikki,$email,$nama,$where_nik,$where_valid,$where_dept,$company_id,$notes=NULL);    
...
$sql2        = mysql_query('SELECT *,'.select_value_sub().' FROM tbl_profile_certification INNER JOIN 
                            ...
                            WHERE '.where_value().' AND bStatus=1 GROUP BY tbl_profile.NIK1 '.having_value().' ORDER BY tbl_apv_group_approval.deptID,iGroupApprovalListId ASC');
...
$mail->Body     = $body;
  ...
  $query  = mysql_query('SELECT *,'.select_value_sub().' FROM tbl_profile_certification 
                        ...
                        WHERE bStatus=1 AND CertNIK'.$where_nik.' AND DeptID'.$where_dept.' AND CompanyId='.$company_id.' AND '.where_value().' '.having_value());
...
function where_value(){
    ...
    return '(CertValidFinish="'.$before_3rd_month.'" OR CertValidFinish="'.$before_2rd_month.'" OR CertValidFinish="'.$before_1rd_month.'" OR CertValidFinish <="'.$current_last_notif.'") AND (CertItem !=3 AND CertItem !=4)';
