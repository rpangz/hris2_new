...
$sql        = mysql_query("SELECT * FROM tbl_profile INNER JOIN 
                    tbl_profile_process ON tbl_profile_process.ProcessId = tbl_profile.ProcessCVNumber 
                    WHERE bStatus=1 AND ProcessStatusForm='P'");
...
while ($data = mysql_fetch_array($sql)){
    ...
    $prosesn        = $data['ProcessLevel'];
    $txtApv         = $data['ProcessApv'.$prosesn];
    $txtTgl         = $data['ProcessDate'.$prosesn];
    $txtNIK         = $data['ProcessNIK'.$prosesn];
    $txtHRD         = $data['ProcessNIK2'];


    $txtPin         = $data['ProcessPin'.$prosesn];
    
    $btn_Apv        = 'ProcessApv'.$prosesn;
    $btn_Tgl        = 'ProcessDate'.$prosesn;
    $btn_NIK        = 'ProcessNIK'.$prosesn;
    $btn_Pin        = 'ProcessPin'.$prosesn;
    $btn_Remark     = $data['ProcessRemark'.$prosesn];

    $txtApvbefore   = $data['ProcessApv'.$proses];

    $session_nik    = $MyNIK;
    
    $last_process   = $data['ProcessLastId'];

    $ProcessRepeated = $data['ProcessRepeated']+1;
    
    //$token          = $txtPin;
    
    if ($prosesn==1){
        $next           = $prosesn;
        $token          = $data['ProcessPin'];
        $alasan         = '';
    }else{
        $next           = $prosesn;
        $token          = $data['ProcessPin1'];
        $alasan         = $data['ProcessRemark1'];
    }

    // Kirim email ke approval terakhir sampai 3 kali masing-masing 1 kali per hari
    if ($total > 0 AND $data['ProcessRepeated'] <= 3){
        $mailto         = mysql_fetch_array(mysql_query("SELECT * FROM tbl_profile WHERE NIK='$txtNIK'"));
        $NIKEmailAdd    = $mailto['Email'];
        $NIKNamaAdd     = $mailto['Nama'];        

        mysql_query("UPDATE tbl_profile_process SET ProcessRepeated = '$ProcessRepeated' WHERE ProcessId = '$id'");

        SendMail($id,$nikki,$next,$last_process,$token,$alasan,$NIKEmailAdd,$NIKNamaAdd);

    }

    // Kirim email ke HRD jika sudah 3 kali tidak di proses
    else{
        $mailto         = mysql_fetch_array(mysql_query("SELECT * FROM tbl_profile WHERE NIK='$txtHRD'"));
        $NIKEmailAdd    = $mailto['Email'];
        $NIKNamaAdd     = $mailto['Nama'];
...
function SendMail($id,$nikki,$next,$last_process,$token,$alasan,$NIKEmailAdd,$NIKNamaAdd){
...
<body id="top">
<p><b>Dear, '.$NIKNamaAdd.'</br>
Form Ini Terhenti Di Anda, Mohon Diproses. Terimakasih</b></p>
...
$mail->Body     .= $body7;
                    ...
                    <tr>
                        ...
                        <td class="organisationnumber"><div align="left"><a href="'.$_SERVER['SERVER_NAME'].'/hris2/modules/karyawan/assets/uploads/'.$ws7['url'].'" target="_blank">Download</a></div></td>                 
                    </tr>';
...
$ApvList = mysql_query("SELECT * FROM tbl_apv_matrik_approval WHERE MatCode='5' AND MatStatus='1'");

    $ban =1;        

    while ($data = mysql_fetch_array($ApvList)){
    
    $sql      = mysql_fetch_array(mysql_query("SELECT * FROM tbl_profile_process WHERE ProcessId='$id'"));  
    $no       = $data['MatProses'];
    $Apv      = $sql['ProcessApv'.$no];
    $Tgl      = $sql['ProcessDate'.$no];
    $NIK      = $sql['ProcessNIK'.$no];
    ...
    <table border=0>
    <tr>
        <td height='18' width='150'>$ban. $data[MatName]</td>
    ...
    $link1          = $url.'ServicesApproval.php?act=accept&id='.$process_id.'&mynik='.$nikki.'&token='.$token.'&proses='.$next.'&last_process='.$last_process;
    $link2          = $url.'ServicesApproval.php?act=reject&id='.$process_id.'&mynik='.$nikki.'&token='.$token.'&proses='.$next.'&last_process='.$last_process;
    $Accept         = 'Accept';
    $Reject         = 'Reject';
...
function jumlah_data($session_nik,$tb_name_temp,$column_name,$last_process,$process_id){

    $tampil     = mysql_query("SELECT * FROM tbl_profile_process WHERE ProcessNIK = '$session_nik' AND ProcessId < '$process_id' ORDER BY ProcessId DESC LIMIT 1");
...
function callback_primary_column_name($table_name_ori){
        $res = mysql_query("SHOW COLUMNS FROM $table_name_ori");
        $row = mysql_fetch_assoc($res);
        return $row['Field'];
}

function callback_primary_column_nik($table_name_ori){
