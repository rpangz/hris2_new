...
*/
	...
	$dws 	    = "SELECT * FROM `tbl_formcuti` INNER JOIN 
				tbl_profile ON tbl_formcuti.FormCutiNIK = tbl_profile.NIK 
				WHERE tbl_profile.bStatus=1 AND tbl_formcuti.StatusForm='P' AND tbl_formcuti.version=0
				ORDER BY tbl_formcuti.FormCutiNIK ASC";
...
function files_cuti($id){
    ...
    while ($data = mysql_fetch_array($file)){
        $link   .= '<a href="https://'.$server_publish.'/hris2/modules/kehadiran/assets/uploads/'.$data['url'].'" target="_blank">'.$data['file_code'].'</a><br/>';
    }
