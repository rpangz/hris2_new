...
$sql = "SELECT * FROM tbl_profile WHERE NIK='$nik' AND";

if (!empty($company_id)){
    $sql .= " CompanyId ='$company_id'";
}
if (!empty($division_id)){
    $sql .= " DivisiID ='$division_id'";
}
if (!empty($department_id)){
    $sql .= " DeptID ='$department_id'";
}
...
while ($row1 = mysql_fetch_array($SQL1)){
  ...
  echo'<table class="zui-table zui-table-horizontal zui-table-highlight">
        ...
        <tr>        
            ...
            <th align="right" width="10%">'.$row1['Qty'].'</th>           
        </tr>
    </thead>
    <tbody>';
