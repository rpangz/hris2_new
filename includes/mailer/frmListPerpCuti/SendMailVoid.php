...
$sqlin = mysql_query("SELECT * FROM tbl_apv_matrik_approval WHERE MatCode='2' AND MatStatus='1' AND companyID='1'");	



	mysql_query("UPDATE tbl_formperpcuti SET StatusForm ='X',
											 ApvLevel = '0' 
											 WHERE FormPerpCutiId = '$_GET[id]'");


while ($dw = mysql_fetch_array($sqlin)){
...
$mail->Body     = $body;
$StatusApproval = 'Permohonan Perpanjangan Sisa Cuti Di VOID dengan alasan '.$Alasan;
...
$ApvList = mysql_query("SELECT * FROM tbl_apv_matrik_approval WHERE MatCode='2' AND MatStatus='1' AND companyID='1'");		
	...
	$NIKEmail 	= $bSQL['NIK'.$data['MatProses']];
