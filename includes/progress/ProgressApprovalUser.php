...
<?php
    ...
    $tampil = mysql_query("SELECT * FROM tbl_formcuti WHERE FormCutiNIK = $_SESSION[NIK] ORDER BY CutiId DESC");    
...
while($data = mysql_fetch_array($tampil)){
	...
	echo "<tr>
        <th bgcolor='$bg'> </th>
        <td>$no.</td>
	   <td bgcolor='' width=180><div align=left><a href='?act=detail&id=$data[CutiId]' class='tooltip-bottom' data-tooltip='$data[Keperluan] [";
...
case"detail":
  ...
  if ($data['StatusForm'] == 'A'){
    ...
    echo "<form method=POST name='submit' action='?act=update&id=$_GET[id]' enctype='multipart/form-data'>
        ...
        <tr>
          <td>Petugas Pengganti</td><td>:</td><td>$data[Pengganti]</td>
        </tr>
        <tr>
...
<br/>
      <table>
        <tr>
        <td width='450'><input type=button value='   Back   ' onclick=window.location.href='?'></td>
        <td  align='right'><input type=button value='   Void   ' onclick=window.location.href='#openModal' $ck></td>
        </tr>
      </table>
      </form>
...
case "void":
  ...
  else 
  {
    ...
    //include "http://astapp02/hris2/includes/mailer/frmCuti/SendMailVoid.php?id=$_GET[id]";
  ...
  <div>
    ...
    <?php echo"<form name='formvoid' id='formvoid' action='?act=void&id=$_GET[id]' onsubmit='return validateComment()' method='POST'>";?>
      <textarea name="Alasan" placeholder="* Harus diisi" style="width: 360px; height: 140px;"></textarea>
        ...
        <input  type="submit" onclick="return validateComment" value="   Submit   ">          
     </form>    
