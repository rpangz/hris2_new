...
//require 'terbilang.php';
	...
	$tampil = mysql_query("SELECT * FROM tbl_formcuti WHERE CutiId = '$_GET[id]' AND StatusForm='A'");		
...
echo"<body>
<a href='javascript:history.back()'><img style=border:0; src='../includes/images/back.png' onmouseover=this.src='../includes/images/back1.png' onmouseout=this.src='../includes/images/back.png' title='Back' width=30 height=30></a>&nbsp;&nbsp;&nbsp;&nbsp;";
echo"<a href='javascript:window.print()';><img style=border:0; src='../includes/images/printer1.png' onmouseover=this.src='../includes/images/printer2.png' onmouseout=this.src='../includes/images/printer1.png' title='Print' width=30 height=30></a>&nbsp;&nbsp;&nbsp;&nbsp;";
//<a href='http://astapp02/e-Kasbon/Com-$_SESSION[CompanyId]/module/mod_$_GET[module]/Email2.php?id=$_GET[id]';><img style=border:0; src='images/Email1.png' onmouseover=this.src='images/Email2.png' onmouseout=this.src='images/Email1.png' title='Email' width=30 height=30></a>&nbsp;&nbsp;&nbsp;&nbsp;";
...
//<a href='#' onClick=window.open('module/Com-$_SESSION[CompanyId]/mod_$_GET[module]/excel2.php?id=$_GET[id]&PRD=$_GET[PRD]&comid=$_SESSION[CompanyId]');><img style=border:0; src='images/excel1.png' onmouseover=this.src='images/excel2.png' onmouseout=this.src='images/excel1.png' title='Excel' width=30 height=30></a>

echo"<p><strong>PERMOHONAN CUTI</strong></p>
<div id='dvContainer'>
<table width=900 class=table1 border=0px cellpadding=0 cellspacing=0 frame=box>
  ...
  <tr >
     <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td colspan=3><div align='center' class=style2>PERMOHONAN CUTI</div></td>
    <td>&nbsp;</td>
  ...
  <tr>
	...
	$List4 = mysql_query("SELECT TOP 3 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND Position='4' ORDER BY iNoProses ASC");		
	$no = 1;
	while ($dta = mysql_fetch_array($List4)){
	
		$sqli = mysql_fetch_array(mysql_query("SELECT * FROM tbProsesApprovalKB WHERE iKBID='$_GET[id]'"));		
		$nu = $dta[iNoProses];
		$Approval = 'iKBNIK'.$nu;
		$cKBApv = $sqli['cKBApv'.$nu];
		$dKBTgl = $sqli['dKBTgl'.$nu];
		$iKBNIK = $sqli['iKBNIK'.$nu];	
		
		$NIKApv = mysql_fetch_array(mysql_query("SELECT * FROM tbProsesApprovalKB WHERE iKBID='$_GET[id]'"));
		...
		elseif ($cKBApv == 'A'){
		$cKBApvStatus = 'Accept';
		$dKBTglName = date('d-M-Y', strtotime($dKBTgl));
		}
		elseif ($cKBApv == 'R'){
		$cKBApvStatus = 'Reject';
		$dKBTglName = date('d-M-Y', strtotime($dKBTgl));
		...
		elseif ($cKBApv == 'C'){
		$cKBApvStatus = 'Checked';
		$dKBTglName = date('d-M-Y', strtotime($dKBTgl));
		}
		elseif ($cKBApv == 'N'){
		$cKBApvStatus = 'Notifikasi';
		$dKBTglName = date('d-M-Y', strtotime($dKBTgl));
		}
		else{
		$cKBApvStatus = '';
		...
		$Divisi = mysql_fetch_array(mysql_query("SELECT * FROM tbApproval WHERE ApprovalId = '$dta[ApprovalId]'"));
		...
		$NIKApv = mysql_fetch_array(mysql_query("SELECT * FROM tbProsesApprovalKB WHERE iKBID='$_GET[id]'"));
		$ProfileName = mysql_fetch_array(mysql_query("SELECT * FROM tblProfileLogin WHERE NIK='$iKBNIK'"));
		$name = explode(" ", $ProfileName[Nama]);		
		$lastname = $name[1];
		if ($name[0] == ""){
		$firstname = '-';
		}
		...
		echo"<tr><td><div align='center' class=style1>
			$Divisi[Name]</br></br>
			$cKBApvStatus</br></br>
			$firstname</div></td>
			</tr>";
  ...
  <tr>
    <td colspan=6 valign='bottom'>
      <table width=100% align=center border=2px cellpadding=0 cellspacing=0 bordercolor=#000000 frame=border>
        <tr>
		<td><div align=center class=style1>Pemohon</div></td>";
	$List1 = mysql_query("SELECT TOP 3 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND Position='2' ORDER BY iNoProses");		
	$no = 1;
	while ($dta = mysql_fetch_array($List1)){
	$Divisi = mysql_fetch_array(mysql_query("SELECT * FROM tbApproval WHERE ApprovalId = '$dta[ApprovalId]'"));	
          echo"<td><div align='center' class=style1>$Divisi[Name]</div></td>";
		  
		  }
                  
         echo"<td><div align='center' class=style1>Penerima Uang</div></td></tr><tr>
		  <td><div align=center><span class=style5><span class=style1></span></span>ttd</div></td>";
	$List2 = mysql_query("SELECT TOP 3 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND Position='2' ORDER BY iNoProses");		
	while ($dta = mysql_fetch_array($List2)){
		...
		$dKBTglName = '';
          ...
          echo"<td height=80px><div align=center><span class=style1>$cKBApvStatus </br>$dKBTglName</div></td>";
         }
		 
		$NIKApv = mysql_fetch_array(mysql_query("SELECT * FROM tbProsesApprovalKB WHERE iKBID='$_GET[id]'"));
		$ProfileName = mysql_fetch_array(mysql_query("SELECT * FROM tblProfileLogin WHERE NIK='$NIKApv[iNIKPemohon]'"));
		$name = explode(" ", $ProfileName[Nama]);
		$firstname = $name[0];
		$lastname = $name[1];
		
        echo"<td><div align=center><span class=style1></div></td></tr><tr>
		 <td><div align=center class=style1>$firstname</div></td>";
	
	$List3 = mysql_query("SELECT TOP 3 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND Position='2' ORDER BY iNoProses");		
	
		while ($dta = mysql_fetch_array($List3)){
			$sqli = mysql_fetch_array(mysql_query("SELECT * FROM tbProsesApprovalKB WHERE iKBID='$_GET[id]'"));
		...
		$firstname = $name[0];
		$lastname = $name[1];
          echo"<td><div align=center class=style1>$firstname</div></td>";
          
		 }
		
        echo"<td><div align=center class=style1>...</div></td>
		</tr></table></table>
		
	</br>
		<hr />";
		
	$sqli 		= mysql_fetch_array(mysql_query("SELECT * FROM tbUHPD WHERE iUHPDKBId = '$_GET[id]'"));
	$dUHPDActive1 = date('d-M-Y', strtotime($sqli[dUHPDActive1]));
	$dUHPDActive2 = date('d-M-Y', strtotime($sqli[dUHPDActive2]));
	
	$dUHPDAct1	= date('d/M', strtotime($sqli[dUHPDActive1]));
	$dUHPDAct2	= date('d/M', strtotime($sqli[dUHPDActive2]));
	$Jobs 		= mysql_fetch_array(mysql_query("SELECT * FROM tbJabatan WHERE CompanyId=$_SESSION[CompanyId] AND JabatanId=$sqli[iUHPDJabatan] ORDER BY NamaJabatan"));
	$Deptku		= mysql_fetch_array(mysql_query("SELECT * FROM tbDept WHERE iCompanyId='$_SESSION[CompanyId]' AND iDeptID='$sqli[iUHPDDiv]'"));
...
<hr />
  ...
  <tr>
	...
	$sql = mysql_query("SELECT * FROM tbCostTrans WHERE CompanyId='$_SESSION[CompanyId]' ORDER BY CostTransId ASC");
	while ($da = mysql_fetch_array($sql)){
	$sqltrans = mysql_fetch_array(mysql_query("SELECT * FROM tbUHPDTrans WHERE iUHPDTransId = '$sqli[iUHPDId]' AND CostTransId=$da[CostTransId]"));
	
	$TotalTrans = $sqltrans[TransPerkiraan] * $sqltrans[AddsTrans];
	
	$TransPerkiraan = number_format($TotalTrans, 2);
	
	if ($da[CostTransId]== $sqltrans[CostTransId]){
	$check = 'Checked';
	}
	else {
	$check = 'disabled';
	}
		echo "<tr><td>&nbsp;$da[CostTransName]</td>
  ...
  <tr>
    <td><table width='900' border='2' cellspacing='0' cellpadding='0' bordercolor=#000000 frame=border>
	  ...
	  $no=1;	
		$sql = mysql_query("SELECT * FROM tbCostDay WHERE CompanyId=$_SESSION[CompanyId] ORDER BY CostDayId ASC"); 
	
	while ($di = mysql_fetch_array($sql)){
		$sqlday = mysql_fetch_array(mysql_query("SELECT * FROM tbUHPDDays WHERE iUHPDDaysId = '$sqli[iUHPDId]' AND CostDayId=$di[CostDayId]"));
		
		$TotalDays = $sqlday[DaysPerkiraan] * $sqlday[AddsDays];
		$DaysPerkiraan = number_format($TotalDays, 2);
	
		if ($di[CostDayId]== $sqlday[CostDayId]){
			$check = 'Checked';
		}
		else {
			$check = 'disabled';
		}
		echo "<tr><td>&nbsp;$di[CostDayName]</td>
		<td>&nbsp;$DaysPerkiraan</td>
		<td></td>
		<td>&nbsp;</td>
        <td>&nbsp;</td>
  ...
  <tr>
    <td><table width='900' border='2' cellspacing='0' cellpadding='0' bordercolor=#000000 frame=border>
      ...
      <tr>	  
		...
		$List1 = mysql_query("SELECT TOP 4 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND print2UHPD='Y' ORDER BY iNoProses");		
		$no = 1;
	while ($dta = mysql_fetch_array($List1)){
	$Divisi = mysql_fetch_array(mysql_query("SELECT * FROM tbApproval WHERE ApprovalId = '$dta[ApprovalId]'"));	
          echo"<td><div align='center'>$Divisi[Name]</div></td>";
		  
		  }		
        echo"<td><div align='center'>Karyawan</div></td>";
		$List1 = mysql_query("SELECT TOP 4 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND print2UHPD='Y' ORDER BY iNoProses");		
		$no = 1;
	while ($dta = mysql_fetch_array($List1)){
	$Divisi = mysql_fetch_array(mysql_query("SELECT * FROM tbApproval WHERE ApprovalId = '$dta[ApprovalId]'"));	
      ...
      echo"</tr><tr><td><div align='center'>&nbsp;ttd</div></td>";
		$List2 = mysql_query("SELECT TOP 4 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND print2UHPD='Y' ORDER BY iNoProses");		
	while ($dta = mysql_fetch_array($List2)){	
		$sqli = mysql_fetch_array(mysql_query("SELECT * FROM tbProsesApprovalKB WHERE iKBID='$_GET[id]'"));		
		$nu = $dta[iNoProses];
		$Approval = 'iKBNIK'.$nu;
	...
	while ($dta = mysql_fetch_array($List2)){
		...
		$dKBTglName = '';
          ...
          echo"<td height=80px><div align=center></div></td>";
         }
		
		
		$name = explode(" ", $data[cKBUser]);
		$firstname = $name[0];
		$lastname = $name[1];
		
      echo"</tr><tr>
        <td><div align='center'>$firstname</div></td>";
		$List3 = mysql_query("SELECT TOP 4 * FROM tbMatrikApproval WHERE iKasbonId ='$data[iKBKasbonId]' AND iCompanyId ='$_SESSION[CompanyId]' AND print2UHPD='Y' ORDER BY iNoProses");		
		...
		$firstname = $name[0];
		$lastname = $name[1];
          echo"<td><div align=center>$firstname</div></td>";
