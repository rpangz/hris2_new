...
switch($_GET[act]){
	...
	echo"<table width=100%>Agenda<hr width=100%></table>";
	echo"<table width='80%'> 
  <tr><td>Period</td><td>:</td><td><select id=select1 onchange=goToPage('select1')>
	...
	$sql = mysql_query("SELECT * FROM tblPeriod ORDER BY id ASC");					  
		  while ($data = mysql_fetch_array($sql)){		  
			echo "<option value='?module=frmAgenda&act=bSearch&PRD=$data[id]'>$data[cName]</option>";
		  }
...
echo"<table width=100%>Agenda >> $period[cName]<hr width=100%></table>";
echo"<table width='80%'> 
  <tr><td>Period</td><td>:</td><td><select id=select1 onchange=goToPage('select1')>";
    $sql = mysql_query("SELECT * FROM tblPeriod ORDER BY id ASC");					  
		  while ($data = mysql_fetch_array($sql)){
		  if ($data[id] == $_GET[PRD]){
			echo "<option value='?module=frmAgenda&act=bSearch&PRD=$data[id]' SELECTED>$data[cName]</option>";
			}
			else{
			echo "<option value='?module=frmAgenda&act=bSearch&PRD=$data[id]'>$data[cName]</option>";
		  }
...
echo"<table width=100%><hr width=100%></table>";
echo "<input type=button value='   Add   ' onclick=\"window.location.href='?module=frmAgenda&act=tambah&PRD=$_GET[PRD]';\"></br>";
$Periode=mysql_fetch_array(mysql_query("SELECT * FROM tblPeriod WHERE id='$_GET[PRD]'"));	

$tampil=mysql_query("SELECT * FROM tblAgenda WHERE (Tanggal >='$Periode[YearId]-$Periode[MonthId]-01' AND Tanggal <='$Periode[YearId]-$Periode[MonthId]-31') ORDER BY Tanggal ASC");	
...
echo"Tidak ada Data";
	...
	for (var i=1;i<tfrow;i++) {
		...
		echo "<tr>
		<td rowspan=1 bgcolor='$void'><div align='center'>$no</td>		
		<td rowspan=1><div align='center'>$Tgl</td>
		<td><div align='left'><a href=?module=frmAgenda&act=Detail&id=$data[id]>$data[Topik]</a></td>	
		<td rowspan=1><div align='center'>$data[User]</td>
...
echo"<table width=100%>Agenda >> $period[cName]<hr width=100%></table>";
	echo"
	<form method=POST action='?module=frmAgenda&act=input'>
	...
	<tr valign='top'><td>Isi</td><td>:</td><td> <textarea class='ckeditor' name='Isi' style='width: 550px; height: 250px;'></textarea></td></tr>
	<tr><td>PIC</td><td>:</td><td><input type=text name='User' size=50</td></tr>
	";
...
if ($_POST[Tanggal]=="0" || $_POST[Topik]=="" || $_POST[Isi]=="" || $_POST[User]=="")
{
...
$Tambah =  mysql_fetch_array(mysql_query("SELECT max(id) AS No FROM tblagenda"));
$a=1;
$New=($Tambah[No] + $a);
...
mysql_query("INSERT INTO tblagenda (id,Tanggal,Topik,Isi,User) 
							VALUES ('$New','$Tgl','$_POST[Topik]','$_POST[Isi]','$_POST[User]')");
...
echo"<table width=100%>Agenda >> Edit<hr width=100%></table>";
	echo"
	<form method=POST action='?module=frmAgenda&act=Update'>
	...
	<tr valign='top'><td>Isi</td><td>:</td><td> <textarea class='ckeditor' name='Isi' style='width: 550px; height: 450px;'>$data[Isi]</textarea></td></tr>
	<tr><td>PIC</td><td>:</td><td><input type=text name='User' size=50 value='$data[User]'</td></tr>
	";
	
	echo"<table width=100%>
	<tr>
	<td colspan=8><input name='simpan' type='submit' value='   Save   '>&nbsp;&nbsp;<input type=button value=' Cancel ' onclick=self.history.back()></div></td>
	<td><input type=button value=' Delete ' onclick=\"window.location.href='?module=frmAgenda&act=Delete&id=$_GET[id]';\"></td>
	</tr></table></form>";
	echo"<table width=100%><hr width=100%></table>";
	break;
	...
	case "Update":
	$Tgl = date('Y-m-d', strtotime($_POST[Tanggal]));
	mysql_query("UPDATE tblagenda SET Tanggal = '$Tgl',
										Topik = '$_POST[Topik]',
										Isi = '$_POST[Isi]',
										User = '$_POST[User]'
									WHERE id='$_POST[id]'");																
	...
	case "Delete":
	mysql_query("DELETE FROM tblagenda WHERE id='$_GET[id]'");																
