...
<td><button type="submit" class="btn btn-primary" name="submit">&nbsp;&nbsp;      View      &nbsp;&nbsp;</button>

<!--<input  class="btn btn-default" type="submit" name="submit" value=" View " >--></td>
</form>
...
for ($i=0; $i<($maxday+$startday); $i++) {
	...
	if($i < $startday) {echo "<td class='calendarTd'><div id='dayNum'> </div></td>\n";}
		...
		$tampil     = mysql_query("SELECT * FROM tbl_formCuti 
									...
									WHERE TglCuti = '$full_date' AND StatusForm='A' AND FormCutiNIK=$_SESSION[NIK] ORDER BY TglCuti");
		...
		while($data = mysql_fetch_array($tampil)){
			...
			echo"&#8727; <a class='tooltips' href='#'>$Topik<span>
