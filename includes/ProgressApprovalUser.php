...
*/
    ...
    ol.progtrckr[data-progtrckr-steps="4"] li { width: 30%; }
    ol.progtrckr[data-progtrckr-steps="5"] li { width: 19%; }
    ol.progtrckr[data-progtrckr-steps="6"] li { width: 16%; }
    ol.progtrckr[data-progtrckr-steps="7"] li { width: 14%; }
    ol.progtrckr[data-progtrckr-steps="8"] li { width: 12%; }
...
// Form Cuti
$tampil = mysql_query("SELECT * FROM tbl_formcuti WHERE FormCutiNIK ='".$session_nik."' AND StatusForm ='P' ORDER BY CutiId DESC");    
...
while($data = mysql_fetch_array($tampil)){
	...
	echo '<tr>
        <th width="4" bgcolor="'.status_form_bg($data['StatusForm']).'"></th>
        <td>'.$no.'.</td>
	      <td bgcolor="" width="180"><div align=left><a href="?act=detail_cuti&id='.$data['CutiId'].'" class="tooltip-bottom" data-tooltip="'.$data['Keperluan'].' [';
...
$tampil_ijin = mysql_query("SELECT * FROM tbl_formijin WHERE NIK ='".$session_nik."' AND StatusForm ='P' ORDER BY IjinId DESC");    
$total_ijin  = mysql_num_rows($tampil_ijin);
...
$no =1;
while($data2 = mysql_fetch_array($tampil_ijin)){

  echo '<tr>
        <th width="4" bgcolor="'.status_form_bg($data2['StatusForm']).'"></th>
        <td>'.$no.'.</td>
       <td bgcolor="" width="180"><div align=left><a href="?act=detail_ijin&id='.$data2['IjinId'].'" class="tooltip-bottom" data-tooltip="'.$data2['Alasan'].'';   

  echo '">'.$data2['Alasan'].'</a></td>  
  <td>
      <ol class="progtrckr" data-progtrckr-steps="2">
          <li class="'.tooltip_class ($value=1,$table='tbl_formijin',$primary_column='IjinId',$data2['IjinId']).'"><a href="#" class="tooltip-bottom" data-tooltip="'.tooltip_text ($data2['Apv1'],$data2['NIK1'],$table='tbl_formijin',$primary_column='IjinId',$data2['IjinId'],$process=1).'">Atasan Langsung</a></li>
          <li class="'.tooltip_class ($value=2,$table='tbl_formijin',$primary_column='IjinId',$data2['IjinId']).'"><a href="#" class="tooltip-bottom" data-tooltip="'.tooltip_text ($data2['Apv2'],$data2['NIK2'],$table='tbl_formijin',$primary_column='IjinId',$data2['IjinId'],$process=2).'">HRD</a></li>          
      </ol>
  </td>  
  </tr>';
...
$tampil = mysql_query("SELECT * FROM tbl_formperpcuti WHERE NIK ='".$session_nik."' AND StatusForm ='P' ORDER BY FormPerpCutiId DESC");    
$total  = mysql_num_rows($tampil);
...
while($data = mysql_fetch_array($tampil)){
  ...
  echo '<tr>
        <th width="4" bgcolor="'.status_form_bg($data['StatusForm']).'"></th>
        <td>'.$no.'.</td>
       <td bgcolor="" width="180"><div align=left><a href="?act=detail_sisa&id='.$data['FormPerpCutiId'].'" class="tooltip-bottom" data-tooltip="'.periode_cuti($data['HakCutiId']).'';

     

  echo '">'.periode_cuti($data['HakCutiId']).'</a></td>  
  <td>
      <ol class="progtrckr" data-progtrckr-steps="2">
...
//$tampil = mysql_query("SELECT * FROM tbl_profile_process WHERE ProcessNIK='".$session_nik."' AND ProcessStatusForm='P' ORDER BY ProcessId DESC LIMIT 1");

$tampil = mysql_query("select a1.* from tbl_profile_process a1 inner join (select max(ProcessId) as max from tbl_profile_process group by ProcessNIK) a2 on a1.ProcessId = a2.max WHERE ProcessStatusForm='P' AND ProcessNIK='".$session_nik."'");
...
while($data = mysql_fetch_array($tampil)){
  ...
  echo '<tr>
        <th width="4" bgcolor="'.status_form_bg($data['ProcessStatusForm']).'"></th>
        <td>'.$no.'.</td>
       <td bgcolor="" width="180"><div align=left><a href="http://'.$_SERVER['SERVER_NAME'].'/hris2/karyawan/frmMyCV/index/edit/'.$data['ProcessNIK'].'/?state=last_update&id='.$data['ProcessId'].'" class="tooltip-bottom" data-tooltip="'.$data['ProcessNIK'].'  '.$session_name.'';

     

  echo '">'.$session_name.'</a></td>  
  <td>
      <ol class="progtrckr" data-progtrckr-steps="2">
...
case"detail_cuti":    
  ...
  if ($data['StatusForm'] == 'A'){
    ...
    elseif ($today >= $TglMasuk){
        $bg = '#FF00FF';
        $st = 'EXPIRED, Please Contact Your HRD'; 
        $ck = 'DISABLED';         
    }    
    else {
        $bg = '';
  ...
  echo '
        <form method=POST name="submit" action="?act=update&id='.$_GET['id'].'" enctype="multipart/form-data">
        ...
        <tr>
          <td>Tanggal Masuk</td><td>:</td><td>'.date_format_id($data['TglMasuk']).'</td>
        </tr>';
...
echo'</table>
    <br/>
      <table>
        <tr>
        <td width="450"><input type="button" class="btn btn-primary" value="&nbsp;&nbsp;&nbsp; Back &nbsp;&nbsp;" onclick=window.location.href="?"></td>
...
<br/>
      <table>
        <tr>
        <td width="450"><input type="button" class="btn btn-primary" value="&nbsp;&nbsp;&nbsp; Back &nbsp;&nbsp;" onclick=window.location.href="?"></td>
        <td  align="right"><input type="button" class="btn btn-default" value="&nbsp;&nbsp;&nbsp; Void &nbsp;&nbsp;" onclick=window.location.href="#openModal_ijin" '.$ck.'></td>
        </tr>
      </table>
      </form>';
...
function date_format_id($value){
  ...
  if (is_null($value) || empty($value)){
    ...
    $foramt = date('d-M-Y', strtotime($value));
  }
