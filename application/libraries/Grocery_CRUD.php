...
class grocery_CRUD_Field_Types
{
	...
	public function get_field_types()
	{
		...
		foreach($this->basic_model->get_field_types_basic_table() as $field_info)
		{
			...
			// Help Block by Dompak
			$field_info->help_block =
				isset($this->help_block[$field_info->name]) ?
					$this->help_block[$field_info->name] :
					ucfirst(str_replace("_"," ",''));


			// placeholder by Dompak
			$field_info->placeholder =
				isset($this->placeholder[$field_info->name]) ?
					$this->placeholder[$field_info->name] :
					ucfirst(str_replace("_"," ",''));
		...
		if(!empty($this->relation_n_n))
		{
			foreach($this->relation_n_n as $field_name => $field_extras)
			{
				...
				$field_info->help_block =
					isset($this->help_block[$field_name]) ?
						$this->help_block[$field_name] :
						ucfirst(str_replace("_"," ",''));


				$field_info->placeholder =
					isset($this->placeholder[$field_name]) ?
			...
			foreach($this->add_fields as $field_object)
			{
				...
				if(!isset($types[$field_name]))//Doesn't exist in the database? Create it for the CRUD
				{
					...
					$field_info = (object)array(
						...
						'help_block' => isset($this->help_block[$field_name]) ?
												$this->help_block[$field_name] :
												ucfirst(str_replace("_"," ",'')),

						'placeholder' => isset($this->placeholder[$field_name]) ?
												$this->placeholder[$field_name] :
												ucfirst(str_replace("_"," ",'')),
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->profile_options = $this->config->profile_options;

		$ci = &get_instance();

		$data->default_profile = $ci->uri->segment(3);
	...
	protected function get_string_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' placeholder='{$field_info->placeholder}' value=\"$value\" $extra_attributes />";
		return $input;
	}
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}' class='$class_name' placeholder='{$field_info->placeholder}' >$value</textarea>";
		}
		else
		{
			$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}' placeholder='{$field_info->placeholder}' >$value</textarea>";
		}
		return $input;
	}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $dialog_forms		    = '';

	protected $form_validation		= null;
	protected $change_field_type	= null;
	protected $primary_keys			= array();
	protected $upload_configs		= array();
	...
	public function set_config_upload($upload_type, $upload_size)
	{		
		$this->upload_configs[] = array('upload_type' => $upload_type, 'upload_size' => $upload_size);

		return $this;
	}

	public function set_dialog_forms($status)
	{		
		$this->dialog_forms = $status;
	 ...
	 * @return void
	 */
	public function help_block($field_name, $help_block = null)
	{
		if(is_array($field_name))
		{
			foreach($field_name as $field => $help_block)
			{
				$this->help_block[$field] = $help_block;
			}
		}
		elseif($help_block !== null)
		{
			$this->help_block[$field_name] = $help_block;
		}
		return $this;
	}
	 ...
	 * @return void
	 */
	public function placeholder($field_name, $placeholder = null)
	{
		if(is_array($field_name))
		{
			foreach($field_name as $field => $placeholder)
			{
		...
		{
			$this->placeholder[$field_name] = $placeholder;
		}
		return $this;
	}
...
//@todo ui_day, ui_month, ui_year has to be lang strings
		...
		if($this->add_fields_checked === false)
		{
			...
			if(!empty($this->add_fields))
			{
				foreach($this->add_fields as $field_num => $field)
				{
					...
					if(isset($this->help_block[$field]))
						$this->help_block[$field_num] = (object)array('field_name' => $field, 'help_block' => $this->help_block[$field]);

					if(isset($this->placeholder[$field]))
						$this->placeholder[$field_num] = (object)array('field_name' => $field, 'placeholder' => $this->placeholder[$field]);
			...
			else
			{
				...
				foreach($field_types as $field)
				{
					...
					if( (!isset($field->db_extra) || $field->db_extra != 'auto_increment') )
					{
						...
						if(isset($this->help_block[$field->name]))
							$this->add_fields[] = (object)array('field_name' => $field->name, 'help_block' => $this->help_block[$field->name]);

						if(isset($this->placeholder[$field->name]))
							$this->add_fields[] = (object)array('field_name' => $field->name, 'placeholder' => $this->placeholder[$field->name]);
		...
		if($this->edit_fields_checked === false)
		{
			...
			if(!empty($this->edit_fields))
			{
				foreach($this->edit_fields as $field_num => $field)
				{
					...
					if(isset($this->help_block[$field]))
						$this->edit_fields[$field_num] = (object)array('field_name' => $field, 'help_block' => $this->help_block[$field]);

					if(isset($this->placeholder[$field]))
						$this->edit_fields[$field_num] = (object)array('field_name' => $field, 'placeholder' => $this->placeholder[$field]);
			...
			else
			{
				...
				foreach($field_types as $field)
				{
					...
					if(!isset($field->db_extra) || $field->db_extra != 'auto_increment')
					{
						...
						if(isset($this->help_block[$field->name]))
							$this->edit_fields[] = (object)array('field_name' => $field->name, 'help_block' => $this->help_block[$field->name]);

						if(isset($this->placeholder[$field->name]))
							$this->edit_fields[] = (object)array('field_name' => $field->name, 'placeholder' => $this->placeholder[$field->name]);
	...
	protected function _initialize_variables()
	{
		...
		// Config for specific upload by Haseamada
		if(!empty($this->upload_configs))
		{
			foreach($this->upload_configs as $upload_config){
				$upload_type = $upload_config['upload_type'];
				$upload_size = $upload_config['upload_size'];
			}

			$this->config->file_upload_allow_file_types	= $upload_type;
			$this->config->file_upload_max_file_size	= $upload_size;
		}
		else{
			$this->config->file_upload_allow_file_types	= $ci->config->item('grocery_crud_file_upload_allow_file_types');
			$this->config->file_upload_max_file_size	= $ci->config->item('grocery_crud_file_upload_max_file_size');
		...
		if($this->dialog_forms === false)
		{
			$this->config->dialog_forms		= $this->dialog_forms;
		}
		else{
		...
		$this->config->profile_options		= $ci->config->item('grocery_crud_profile_options');
