...
class CMS_Model extends CI_Model
{
    ...
    private function __update(){
        ...
        if($old_version !== $current_version){
            ...
            $query = $this->db->select('authorization_id')
                ->from('tbl_main_authorization')
                ->where('authorization_id',5)
                ->get();
            if($query->num_rows() == 0){
                $this->db->insert('tbl_main_authorization', array(
                    'authorization_id'=>5,
            ...
            $table_name = 'tbl_main_navigation';
    ...
    public function __construct()
    {
        ...
        $this->load->helper('bank_mandiri_helper');
    ...
    public function cms_user_group(){
        ...
        */
        ...
        $query = $this->db->select('group_name')
            ->from('tbl_main_group')
            ->join('tbl_main_group_user','tbl_main_group_user'.'.group_id = '.'tbl_main_group'.'.group_id')
            ->where('tbl_main_group_user'.'.user_id', $this->cms_user_id())
            ->get();
        ...
        $query         = $this->db->query("SELECT navigation_id, navigation_name, bootstrap_glyph, is_static, title, description, url, notif_url, active,
                    (
                        ...
                        (
                            (authorization_id = 4 AND $login) AND
                            (
                                (SELECT COUNT(*) FROM tbl_main_group_user AS gu WHERE gu.group_id=1 AND gu.user_id =" . addslashes($user_id) . ")>0
                                    OR $super_user OR
                                (SELECT COUNT(*) FROM tbl_main_group_navigation AS gn
                                    WHERE
                                        gn.navigation_id=n.navigation_id AND
                                        gn.group_id IN
                                            (SELECT group_id FROM tbl_main_group_user WHERE user_id = " . addslashes($user_id) . ")
                                )>0
                            )
                        ) OR
                        (
                ...
                FROM tbl_main_navigation AS n WHERE
                    $where_is_root ORDER BY n.".$this->db->protect_identifiers('index'));
    ...
    public function cms_quicklinks()
    {
        ...
        */
        ...
        $query  = $this->db->query("SELECT q.navigation_id, navigation_name, bootstrap_glyph, is_static, title, description, url, notif_url, active
                        FROM
                            ".'tbl_main_navigation'." AS n,
                            ".'tbl_main_quicklink'." AS q
                        WHERE
                            (
                                q.navigation_id = n.navigation_id
                            )
    ...
    public function cms_widgets($slug = NULL, $widget_name=NULL)
    {
        ...
        $slug_where = isset($slug)?
            ...
            "1=1";
        $widget_name_where = isset($widget_name)? "widget_name LIKE '".addslashes($widget_name)."' AND active=1 " : "1=1";

        $SQL = "SELECT
                    widget_id, widget_name, is_static, title,
                    description, url, slug, static_content,is_span
                    ...
                    (
                        ...
                        (
                            ...
                            (
                                (SELECT COUNT(*) FROM tbl_main_group_user AS gu WHERE gu.group_id=1 AND gu.user_id ='" . addslashes($user_id) . "')>0
                                    OR $super_user OR
                                (SELECT COUNT(*) FROM tbl_main_group_widget AS gw
                                    WHERE
    ...
    public function cms_submenu_screen($navigation_name)
    {
        ...
        if (!isset($navigation_name)) {
            ...
            $query = $this->db->select('navigation_id')->from('tbl_main_navigation')->where('navigation_name', $navigation_name)->get();
            if ($query->num_rows() == 0) {
                return '';
            } else {
                $row           = $query->row();
    ...
    public function cms_do_login($identity, $password)
    {
        ...
        if($login_succeed){
            ...
            if(!isset($_SESSION['__cms_user_read_welcome_alert'])){
                $_SESSION['__cms_user_read_welcome_alert'] = FALSE;
            }
    ...
    public function cms_do_logout()
    {
        ...
        $this->cms_unset_ci_session('__cms_user_read_welcome_alert');
        // needed by kcfinder
        if(isset($_SESSION)){
