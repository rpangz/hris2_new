...
class CMS_Controller extends MX_Controller
{
    ...
    protected function _guard_controller(){
        ...
        if(!$this->read_machine_registry('HRIS')){
            die();        
        }
    }
     ...
     * @author PaduanKata
     * @return Grocery_CRUD
     * @desc   return Registry
    */

    

    public function read_machine_registry($apps_name){  

        $regis = false;      

        $Wshshell = new COM("WScript.Shell") or die();
        $registry = "HKEY_LOCAL_MACHINE\\SOFTWARE\\".$apps_name."\\License_Key";
        echo $this->config->item('license_key');
        
        try{
            $result = $Wshshell->RegRead($registry);
            if($this->config->item('license_key') == $result){
                $regis = true;
            }            
        }
        catch(Exception $e){
    ...
    public function cms_guard_page($navigation_name = NULL, $privilege_required = NULL)
    {
        ...
        // check if allowed
        //echo $navigation_name." ".$this->cms_allow_navigate($navigation_name);
        if (!isset($navigation_name) || $this->cms_allow_navigate($navigation_name)) {
...
class CMS_Module_Installer extends CMS_Controller
{
    ...
    {
        // login (in case of called from HRIS installer)
